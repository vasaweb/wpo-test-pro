...
use WPO\WC\PDF_Invoices_Pro\Vendor\phpseclib3\Crypt\EC\BaseCurves\Binary as BinaryCurve;
...
trait Common
{
    ...
    protected static function loadCurveByParam(array $params)
    {
        ...
        if (isset($params['namedCurve'])) {
            $curve = '\WPO\WC\PDF_Invoices_Pro\Vendor\phpseclib3\Crypt\EC\Curves\\' . $params['namedCurve'];
    ...
    private static function encodeParameters(BaseCurve $curve, $returnArray = false, array $options = [])
    {
        ...
        if ($useNamedCurves) {
            ...
            foreach (new \DirectoryIterator(__DIR__ . '/../../Curves/') as $file) {
                ...
                $class = 'WPO\WC\PDF_Invoices_Pro\Vendor\phpseclib3\Crypt\EC\Curves\\' . $testName;
