...
use WPO\WC\PDF_Invoices_Pro\Vendor\phpseclib3\Exception\BadDecryptionException;
use WPO\WC\PDF_Invoices_Pro\Vendor\phpseclib3\Exception\BadModeException;
use WPO\WC\PDF_Invoices_Pro\Vendor\phpseclib3\Exception\InconsistentSetupException;
...
use WPO\WC\PDF_Invoices_Pro\Vendor\phpseclib3\Math\BinaryField;
use WPO\WC\PDF_Invoices_Pro\Vendor\phpseclib3\Math\PrimeField;
...
abstract class SymmetricKey
{
     ...
     * Engine Reverse Map
     *
     * @see \WPO\WC\PDF_Invoices_Pro\Vendor\phpseclib3\Crypt\Common\SymmetricKey::getEngine()
     */
    const ENGINE_MAP = [
     ...
     *
     * @see \WPO\WC\PDF_Invoices_Pro\Vendor\phpseclib3\Crypt\Twofish::setKey()
     * @see \WPO\WC\PDF_Invoices_Pro\Vendor\phpseclib3\Crypt\Twofish::setIV()
     * @var bool
     */
                    ...
                    if (strlen($_buffer["ciphertext"])) {
                        for ($_i = 0; $_i < $_plaintext_len; $_i+= ' . $block_size . ') {
                            ...
                            if (strlen($_block) > strlen($_buffer["ciphertext"])) {
                                $in = $_xor;
                                ' . $encrypt_block . '
                                \WPO\WC\PDF_Invoices_Pro\Vendor\phpseclib3\Common\Functions\Strings::increment_str($_xor);
                                $_buffer["ciphertext"].= $in;
                            }
                            $_key = \WPO\WC\PDF_Invoices_Pro\Vendor\phpseclib3\Common\Functions\Strings::shift($_buffer["ciphertext"], ' . $block_size . ');
                ...
                $encrypt = $init_encrypt . '
                    ...
                    if (strlen($_buffer["xor"])) {
                        for ($_i = 0; $_i < $_plaintext_len; $_i+= ' . $block_size . ') {
                            ...
                            $_key = \WPO\WC\PDF_Invoices_Pro\Vendor\phpseclib3\Common\Functions\Strings::shift($_buffer["xor"], ' . $block_size . ');
                            $_ciphertext.= $_block ^ $_key;
                        }
                    } else {
                        for ($_i = 0; $_i < $_plaintext_len; $_i+= ' . $block_size . ') {
