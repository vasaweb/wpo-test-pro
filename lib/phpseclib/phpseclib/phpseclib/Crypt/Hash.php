 ...
 *
 *    $hash = new \WPO\WC\PDF_Invoices_Pro\Vendor\phpseclib3\Crypt\Hash('sha512');
 *
 *    $hash->setKey('abcdefg');
 *
...
class Hash
{
    ...
    public function setHash($hash)
    {
        ...
        if (in_array(substr($hash, 0, 5), ['sha3-', 'shake', 'kecca'])) {
            ...
            if (version_compare(PHP_VERSION, '7.1.0') < 0 || substr($hash, 0, 5) != 'sha3-') {
                ...
                $hash = ['WPO\WC\PDF_Invoices_Pro\Vendor\phpseclib3\Crypt\Hash', PHP_INT_SIZE == 8 ? 'sha3_64' : 'sha3_32'];
            }
        }
        ...
        if ($hash == 'sha512/224' || $hash == 'sha512/256') {
            ...
            if (version_compare(PHP_VERSION, '7.1.0') < 0) {
                ...
                $hash = ['WPO\WC\PDF_Invoices_Pro\Vendor\phpseclib3\Crypt\Hash', 'sha512'];
            }
        }
    ...
    private static function sha512($m, $hash)
    {
        ...
        // (\WPO\WC\PDF_Invoices_Pro\Vendor\phpseclib3\Crypt\Hash::hash() trims the output for hashes but not for HMACs.  as such, we trim the output here)
