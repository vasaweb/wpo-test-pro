...
//
// This file is part of WPO_WCPDF_IPS_PRO_TCPDF software library.
//
// WPO_WCPDF_IPS_PRO_TCPDF is free software: you can redistribute it and/or modify it
// under the terms of the GNU Lesser General Public License as
// published by the Free Software Foundation, either version 3 of the
// License, or (at your option) any later version.
//
// WPO_WCPDF_IPS_PRO_TCPDF is distributed in the hope that it will be useful, but
// WITHOUT ANY WARRANTY; without even the implied warranty of
// MERCHANTABILITY or FITNESS FOR A PARTICULAR PURPOSE.
// See the GNU Lesser General Public License for more details.
//
// You should have received a copy of the License
// along with WPO_WCPDF_IPS_PRO_TCPDF. If not, see
...
//   Domain WPO_WCPDF_IPS_PRO_FPDF class by Olivier Plathey (http://www.fpdf.org),
...
//  * 1D and 2D barcodes: CODE 39, ANSI MH10.8M-1983, USD-3, 3 of 9, CODE 93, USS-93, Standard 2 of 5, Interleaved 2 of 5, CODE 128 A/B/C, 2 and 5 Digits UPC-Based Extension, EAN 8, EAN 13, UPC-A, UPC-E, MSI, POSTNET, PLANET, RMS4CC (Royal Mail 4-state Customer Code), CBC (Customer Bar Code), KIX (Klant index - Customer index), Intelligent Mail Barcode, Onecode, USPS-B-3200, CODABAR, CODE 11, PHARMACODE, PHARMACODE TWO-TRACKS, WPO_WCPDF_IPS_PRO_Datamatrix, QR-Code, WPO_WCPDF_IPS_PRO_PDF417;
 ...
 * WPO_WCPDF_IPS_PRO_TCPDF project (http://www.tcpdf.org) was originally derived in 2002 from the Public Domain WPO_WCPDF_IPS_PRO_FPDF class by Olivier Plathey (http://www.fpdf.org), but now is almost entirely rewritten.<br>
 * <h3>WPO_WCPDF_IPS_PRO_TCPDF main features are:</h3>
 * <ul>
...
// WPO_WCPDF_IPS_PRO_TCPDF configuration
require_once(dirname(__FILE__).'/tcpdf_autoconfig.php');
// WPO_WCPDF_IPS_PRO_TCPDF static font methods and data
require_once(dirname(__FILE__).'/include/tcpdf_font_data.php');
// WPO_WCPDF_IPS_PRO_TCPDF static font methods and data
require_once(dirname(__FILE__).'/include/tcpdf_fonts.php');
// WPO_WCPDF_IPS_PRO_TCPDF static color methods and data
require_once(dirname(__FILE__).'/include/tcpdf_colors.php');
// WPO_WCPDF_IPS_PRO_TCPDF static image methods and data
require_once(dirname(__FILE__).'/include/tcpdf_images.php');
// WPO_WCPDF_IPS_PRO_TCPDF static methods and data
require_once(dirname(__FILE__).'/include/tcpdf_static.php');

// - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -

/**
 * @class WPO_WCPDF_IPS_PRO_TCPDF
 * PHP class for generating PDF documents without requiring external extensions.
 * WPO_WCPDF_IPS_PRO_TCPDF project (http://www.tcpdf.org) has been originally derived in 2002 from the Public Domain WPO_WCPDF_IPS_PRO_FPDF class by Olivier Plathey (http://www.fpdf.org), but now is almost entirely rewritten.<br>
 * @package com.tecnick.tcpdf
...
class WPO_WCPDF_IPS_PRO_TCPDF {
	...
	// WPO_WCPDF_IPS_PRO_variables for html parser
	...
	/**
	 * If true print WPO_WCPDF_IPS_PRO_TCPDF meta link.
	 * @protected
	 * @since 5.9.152 (2012-03-23)
	 */
	...
	public function __construct($orientation='P', $unit='mm', $format='A4', $unicode=true, $encoding='UTF-8', $diskcache=false, $pdfa=false) {
		...
		$this->file_id = md5(WPO_WCPDF_IPS_PRO_TCPDF_STATIC::getRandomSeed('WPO_WCPDF_IPS_PRO_TCPDF'.$orientation.$unit.$serformat.$encoding));
		$this->hash_key = hash_hmac('sha256', WPO_WCPDF_IPS_PRO_TCPDF_STATIC::getRandomSeed($this->file_id), WPO_WCPDF_IPS_PRO_TCPDF_STATIC::getRandomSeed('WPO_WCPDF_IPS_PRO_TCPDF'), false);
		...
		WPO_WCPDF_IPS_PRO_TCPDF_FONTS::utf8Bidi(array(), '', false, $this->isunicode, $this->CurrentFont);
		// set default font
	...
	protected function setPageFormat($format, $orientation='P') {
		...
		if (is_string($format)) {
			...
			$pf = WPO_WCPDF_IPS_PRO_TCPDF_STATIC::getPageSizeFromFormat($format);
			$this->fwPt = $pf[0];
			$this->fhPt = $pf[1];
		} else {
			// the boundaries of the physical medium on which the page shall be displayed or printed
			if (isset($format['MediaBox'])) {
				$this->pagedim = WPO_WCPDF_IPS_PRO_TCPDF_STATIC::setPageBoxes($this->page, 'MediaBox', $format['MediaBox']['llx'], $format['MediaBox']['lly'], $format['MediaBox']['urx'], $format['MediaBox']['ury'], false, $this->k, $this->pagedim);
				...
				if (isset($format[0]) AND is_numeric($format[0]) AND isset($format[1]) AND is_numeric($format[1])) {
					...
					$pf = WPO_WCPDF_IPS_PRO_TCPDF_STATIC::getPageSizeFromFormat($format['format']);
				}
				$this->fwPt = $pf[0];
				$this->fhPt = $pf[1];
				$this->pagedim = WPO_WCPDF_IPS_PRO_TCPDF_STATIC::setPageBoxes($this->page, 'MediaBox', 0, 0, $this->fwPt, $this->fhPt, true, $this->k, $this->pagedim);
			}
			// the visible region of default user space
			if (isset($format['CropBox'])) {
				$this->pagedim = WPO_WCPDF_IPS_PRO_TCPDF_STATIC::setPageBoxes($this->page, 'CropBox', $format['CropBox']['llx'], $format['CropBox']['lly'], $format['CropBox']['urx'], $format['CropBox']['ury'], false, $this->k, $this->pagedim);
			}
			// the region to which the contents of the page shall be clipped when output in a production environment
			if (isset($format['BleedBox'])) {
				$this->pagedim = WPO_WCPDF_IPS_PRO_TCPDF_STATIC::setPageBoxes($this->page, 'BleedBox', $format['BleedBox']['llx'], $format['BleedBox']['lly'], $format['BleedBox']['urx'], $format['BleedBox']['ury'], false, $this->k, $this->pagedim);
			}
			// the intended dimensions of the finished page after trimming
			if (isset($format['TrimBox'])) {
				$this->pagedim = WPO_WCPDF_IPS_PRO_TCPDF_STATIC::setPageBoxes($this->page, 'TrimBox', $format['TrimBox']['llx'], $format['TrimBox']['lly'], $format['TrimBox']['urx'], $format['TrimBox']['ury'], false, $this->k, $this->pagedim);
			}
			// the page's meaningful content (including potential white space)
			if (isset($format['ArtBox'])) {
				$this->pagedim = WPO_WCPDF_IPS_PRO_TCPDF_STATIC::setPageBoxes($this->page, 'ArtBox', $format['ArtBox']['llx'], $format['ArtBox']['lly'], $format['ArtBox']['urx'], $format['ArtBox']['ury'], false, $this->k, $this->pagedim);
			}
	...
	public function setPageOrientation($orientation, $autopagebreak=null, $bottommargin=null) {
		...
		if (!isset($this->pagedim[$this->page]['CropBox'])) {
			// the visible region of default user space
			$this->pagedim = WPO_WCPDF_IPS_PRO_TCPDF_STATIC::setPageBoxes($this->page, 'CropBox', $this->pagedim[$this->page]['MediaBox']['llx'], $this->pagedim[$this->page]['MediaBox']['lly'], $this->pagedim[$this->page]['MediaBox']['urx'], $this->pagedim[$this->page]['MediaBox']['ury'], true, $this->k, $this->pagedim);
		}
		if (!isset($this->pagedim[$this->page]['BleedBox'])) {
			// the region to which the contents of the page shall be clipped when output in a production environment
			$this->pagedim = WPO_WCPDF_IPS_PRO_TCPDF_STATIC::setPageBoxes($this->page, 'BleedBox', $this->pagedim[$this->page]['CropBox']['llx'], $this->pagedim[$this->page]['CropBox']['lly'], $this->pagedim[$this->page]['CropBox']['urx'], $this->pagedim[$this->page]['CropBox']['ury'], true, $this->k, $this->pagedim);
		}
		if (!isset($this->pagedim[$this->page]['TrimBox'])) {
			// the intended dimensions of the finished page after trimming
			$this->pagedim = WPO_WCPDF_IPS_PRO_TCPDF_STATIC::setPageBoxes($this->page, 'TrimBox', $this->pagedim[$this->page]['CropBox']['llx'], $this->pagedim[$this->page]['CropBox']['lly'], $this->pagedim[$this->page]['CropBox']['urx'], $this->pagedim[$this->page]['CropBox']['ury'], true, $this->k, $this->pagedim);
		}
		if (!isset($this->pagedim[$this->page]['ArtBox'])) {
			// the page's meaningful content (including potential white space)
			$this->pagedim = WPO_WCPDF_IPS_PRO_TCPDF_STATIC::setPageBoxes($this->page, 'ArtBox', $this->pagedim[$this->page]['CropBox']['llx'], $this->pagedim[$this->page]['CropBox']['lly'], $this->pagedim[$this->page]['CropBox']['urx'], $this->pagedim[$this->page]['CropBox']['ury'], true, $this->k, $this->pagedim);
		}
		...
		if ((abs($this->pagedim[$this->page]['MediaBox']['urx'] - $this->hPt) < $this->feps) AND (abs($this->pagedim[$this->page]['MediaBox']['ury'] - $this->wPt) < $this->feps)){
			...
			$this->pagedim = WPO_WCPDF_IPS_PRO_TCPDF_STATIC::swapPageBoxCoordinates($this->page, $this->pagedim);
		}
		$this->w = ($this->wPt / $this->k);
		$this->h = ($this->hPt / $this->k);
		if (WPO_WCPDF_IPS_PRO_TCPDF_STATIC::empty_string($autopagebreak)) {
		...
		if (WPO_WCPDF_IPS_PRO_TCPDF_STATIC::empty_string($bottommargin)) {
	...
	public function setCellPaddings($left=null, $top=null, $right=null, $bottom=null) {
		if (!WPO_WCPDF_IPS_PRO_TCPDF_STATIC::empty_string($left) AND ($left >= 0)) {
			$this->cell_padding['L'] = $left;
		}
		if (!WPO_WCPDF_IPS_PRO_TCPDF_STATIC::empty_string($top) AND ($top >= 0)) {
			$this->cell_padding['T'] = $top;
		}
		if (!WPO_WCPDF_IPS_PRO_TCPDF_STATIC::empty_string($right) AND ($right >= 0)) {
			$this->cell_padding['R'] = $right;
		}
		if (!WPO_WCPDF_IPS_PRO_TCPDF_STATIC::empty_string($bottom) AND ($bottom >= 0)) {
	...
	public function setDisplayMode($zoom, $layout='SinglePage', $mode='UseNone') {
		...
		$this->LayoutMode = WPO_WCPDF_IPS_PRO_TCPDF_STATIC::getPageLayoutMode($layout);
		$this->PageMode = WPO_WCPDF_IPS_PRO_TCPDF_STATIC::getPageMode($mode);
	}
	...
	public function Error($msg) {
		// unset all class WPO_WCPDF_IPS_PRO_variables
		$this->_destroy(true);
		if (defined('K_TCPDF_THROW_EXCEPTION_ERROR') AND !K_TCPDF_THROW_EXCEPTION_ERROR) {
			die('<strong>WPO_WCPDF_IPS_PRO_TCPDF ERROR: </strong>'.$msg);
		} else {
			throw new Exception('WPO_WCPDF_IPS_PRO_TCPDF ERROR: '.$msg);
		}
	}
	...
	public function Close() {
		...
		// close document
		$this->_enddoc();
		// unset all class WPO_WCPDF_IPS_PRO_variables (except critical ones)
		$this->_destroy(false);
	}
	...
	public function Header() {
		if ($this->header_xobjid === false) {
			...
			if (($headerdata['logo']) AND ($headerdata['logo'] != K_BLANK_IMAGE)) {
				$imgtype = WPO_WCPDF_IPS_PRO_TCPDF_IMAGES::getImageFileType(K_PATH_IMAGES.$headerdata['logo']);
	...
	protected function setTableHeader() {
		...
		if (!WPO_WCPDF_IPS_PRO_TCPDF_STATIC::empty_string($this->thead) AND (!$this->inthead)) {
			// set margins
	...
	public function setSpotColor($type, $name, $tint=100) {
		$spotcolor = WPO_WCPDF_IPS_PRO_TCPDF_COLORS::getSpotColor($name, $this->spot_colors);
		if ($spotcolor === false) {
	...
	public function GetStringWidth($s, $fontname='', $fontstyle='', $fontsize=0, $getarray=false) {
		return $this->GetArrStringWidth(WPO_WCPDF_IPS_PRO_TCPDF_FONTS::utf8Bidi(WPO_WCPDF_IPS_PRO_TCPDF_FONTS::UTF8StringToArray($s, $this->isunicode, $this->CurrentFont), $s, $this->tmprtl, $this->isunicode, $this->CurrentFont), $fontname, $fontstyle, $fontsize, $getarray);
	}
	...
	public function GetArrStringWidth($sa, $fontname='', $fontstyle='', $fontsize=0, $getarray=false) {
		...
		if (!WPO_WCPDF_IPS_PRO_TCPDF_STATIC::empty_string($fontname)) {
		...
		if ($this->isunicode AND (!$this->isUnicodeFont())) {
			$sa = WPO_WCPDF_IPS_PRO_TCPDF_FONTS::UTF8ArrToLatin1Arr($sa);
		}
		$w = 0; // total width
	...
	public function GetNumChars($s) {
		if ($this->isUnicodeFont()) {
			return count(WPO_WCPDF_IPS_PRO_TCPDF_FONTS::UTF8StringToArray($s, $this->isunicode, $this->CurrentFont));
		}
		return strlen($s);
	}
	...
	protected function getFontsList() {
		if (($fontsdir = opendir(WPO_WCPDF_IPS_PRO_TCPDF_FONTS::_getfontpath())) !== false) {
	...
	public function AddFont($family, $style='', $fontfile='', $subset='default') {
		...
		if (WPO_WCPDF_IPS_PRO_TCPDF_STATIC::empty_string($family)) {
			if (!WPO_WCPDF_IPS_PRO_TCPDF_STATIC::empty_string($this->FontFamily)) {
		...
		$fontdir = false;
		if (!WPO_WCPDF_IPS_PRO_TCPDF_STATIC::empty_string($fontfile)) {
			$fontdir = dirname($fontfile);
			if (WPO_WCPDF_IPS_PRO_TCPDF_STATIC::empty_string($fontdir) OR ($fontdir == '.')) {
				$fontdir = '';
			} else {
				$fontdir .= '/';
			}
		...
		if (WPO_WCPDF_IPS_PRO_TCPDF_STATIC::empty_string($fontfile) OR (!@WPO_WCPDF_IPS_PRO_TCPDF_STATIC::file_exists($fontfile))) {
			// build a standard filenames for specified font
			$tmp_fontfile = str_replace(' ', '', $family).strtolower($style).'.php';
			$fontfile = WPO_WCPDF_IPS_PRO_TCPDF_FONTS::getFontFullPath($tmp_fontfile, $fontdir);
			if (WPO_WCPDF_IPS_PRO_TCPDF_STATIC::empty_string($fontfile)) {
				$missing_style = true;
		...
		// include font file
		if (!WPO_WCPDF_IPS_PRO_TCPDF_STATIC::empty_string($fontfile) AND (@WPO_WCPDF_IPS_PRO_TCPDF_STATIC::file_exists($fontfile))) {
			$type=null;
			$name=null;
			$desc=null;
			$up=-null;
		...
		if (!isset($file) OR WPO_WCPDF_IPS_PRO_TCPDF_STATIC::empty_string($file)) {
			$file = '';
		}
		if (!isset($enc) OR WPO_WCPDF_IPS_PRO_TCPDF_STATIC::empty_string($enc)) {
			$enc = '';
		}
		if (!isset($cidinfo) OR WPO_WCPDF_IPS_PRO_TCPDF_STATIC::empty_string($cidinfo)) {
			$cidinfo = array('Registry'=>'Adobe', 'Ordering'=>'Identity', 'Supplement'=>0);
			$cidinfo['uni2cid'] = array();
		}
		if (!isset($ctg) OR WPO_WCPDF_IPS_PRO_TCPDF_STATIC::empty_string($ctg)) {
			$ctg = '';
		}
		if (!isset($desc) OR WPO_WCPDF_IPS_PRO_TCPDF_STATIC::empty_string($desc)) {
			$desc = array();
		}
		if (!isset($up) OR WPO_WCPDF_IPS_PRO_TCPDF_STATIC::empty_string($up)) {
			$up = -100;
		}
		if (!isset($ut) OR WPO_WCPDF_IPS_PRO_TCPDF_STATIC::empty_string($ut)) {
			$ut = 50;
		}
		if (!isset($cw) OR WPO_WCPDF_IPS_PRO_TCPDF_STATIC::empty_string($cw)) {
			$cw = array();
		}
		if (!isset($dw) OR WPO_WCPDF_IPS_PRO_TCPDF_STATIC::empty_string($dw)) {
			// set default width
		...
		if (!WPO_WCPDF_IPS_PRO_TCPDF_STATIC::empty_string($file)) {
	...
	public function isCharDefined($char, $font='', $style='') {
		if (is_string($char)) {
			// get character code
			$char = WPO_WCPDF_IPS_PRO_TCPDF_FONTS::UTF8StringToArray($char, $this->isunicode, $this->CurrentFont);
			$char = $char[0];
		}
		if (WPO_WCPDF_IPS_PRO_TCPDF_STATIC::empty_string($font)) {
			if (WPO_WCPDF_IPS_PRO_TCPDF_STATIC::empty_string($style)) {
	...
	public function replaceMissingChars($text, $font='', $style='', $subs=array()) {
		...
		$fontdata = $this->AddFont($font, $style);
		$fontinfo = $this->getFontBuffer($fontdata['fontkey']);
		$uniarr = WPO_WCPDF_IPS_PRO_TCPDF_FONTS::UTF8StringToArray($text, $this->isunicode, $this->CurrentFont);
		...
		return WPO_WCPDF_IPS_PRO_TCPDF_FONTS::UniArrSubString(WPO_WCPDF_IPS_PRO_TCPDF_FONTS::UTF8ArrayToUniArray($uniarr, $this->isunicode));
	}
	...
	public function Annotation($x, $y, $w, $h, $text, $opt=array('Subtype'=>'Text'), $spaces=0) {
		...
		if (!$this->pdfa_mode || ($this->pdfa_mode && $this->pdfa_version == 3)) {
			if ((($opt['Subtype'] == 'FileAttachment') OR ($opt['Subtype'] == 'Sound')) AND (!WPO_WCPDF_IPS_PRO_TCPDF_STATIC::empty_string($opt['FS']))
				AND (@WPO_WCPDF_IPS_PRO_TCPDF_STATIC::file_exists($opt['FS']) OR WPO_WCPDF_IPS_PRO_TCPDF_STATIC::isValidURL($opt['FS']))
				AND (!isset($this->embeddedfiles[basename($opt['FS'])]))) {
				$this->embeddedfiles[basename($opt['FS'])] = array('f' => ++$this->n, 'n' => ++$this->n, 'file' => $opt['FS']);
			}
		}
		// Add widgets annotation's icons
		if (isset($opt['mk']['i']) AND @WPO_WCPDF_IPS_PRO_TCPDF_STATIC::file_exists($opt['mk']['i'])) {
			$this->Image($opt['mk']['i'], '', '', 10, 10, '', '', '', false, 300, '', false, false, 0, false, true);
		}
		if (isset($opt['mk']['ri']) AND @WPO_WCPDF_IPS_PRO_TCPDF_STATIC::file_exists($opt['mk']['ri'])) {
			$this->Image($opt['mk']['ri'], '', '', 0, 0, '', '', '', false, 300, '', false, false, 0, false, true);
		}
		if (isset($opt['mk']['ix']) AND @WPO_WCPDF_IPS_PRO_TCPDF_STATIC::file_exists($opt['mk']['ix'])) {
	...
	protected function checkPageBreak($h=0, $y=null, $addpage=true) {
		if (WPO_WCPDF_IPS_PRO_TCPDF_STATIC::empty_string($y)) {
			$y = $this->y;
		}
	...
	protected function getCellCode($w, $h=0, $txt='', $border=0, $ln=0, $align='', $fill=false, $link='', $stretch=0, $ignore_min_height=false, $calign='T', $valign='M') {
		...
		$txt = is_null($txt) ? '' : $txt;
		$txt = str_replace(WPO_WCPDF_IPS_PRO_TCPDF_FONTS::unichr(160, $this->isunicode), ' ', $txt);
		$prev_cell_margin = $this->cell_margin;
		$prev_cell_padding = $this->cell_padding;
		$txt = WPO_WCPDF_IPS_PRO_TCPDF_STATIC::removeSHY($txt, $this->isunicode);
		...
		if (WPO_WCPDF_IPS_PRO_TCPDF_STATIC::empty_string($w) OR ($w <= 0)) {
			if ($this->rtl) {
				$w = $x - $this->lMargin;
			} else {
		...
		if ($txt != '') {
			...
			if ($this->isunicode) {
				if (($this->CurrentFont['type'] == 'core') OR ($this->CurrentFont['type'] == 'TrueType') OR ($this->CurrentFont['type'] == 'Type1')) {
					$txt2 = WPO_WCPDF_IPS_PRO_TCPDF_FONTS::UTF8ToLatin1($txt2, $this->isunicode, $this->CurrentFont);
				} else {
					$unicode = WPO_WCPDF_IPS_PRO_TCPDF_FONTS::UTF8StringToArray($txt, $this->isunicode, $this->CurrentFont); // array of UTF-8 unicode values
					$unicode = WPO_WCPDF_IPS_PRO_TCPDF_FONTS::utf8Bidi($unicode, '', $this->tmprtl, $this->isunicode, $this->CurrentFont);
					...
					$txt2 = WPO_WCPDF_IPS_PRO_TCPDF_FONTS::arrUTF8ToUTF16BE($unicode, false);
				}
			}
			$txt2 = WPO_WCPDF_IPS_PRO_TCPDF_STATIC::_escape($txt2);
			...
			if (isset($uniblock)) { // @phpstan-ignore-line
				...
				foreach ($uniblock as $uk => $uniarr) { // @phpstan-ignore-line
					if (($uk % 2) == 0) {
						...
						// character to print
						$topchr = WPO_WCPDF_IPS_PRO_TCPDF_FONTS::arrUTF8ToUTF16BE($uniarr, false);
						$topchr = WPO_WCPDF_IPS_PRO_TCPDF_STATIC::_escape($topchr);
	...
	public function MultiCell($w, $h, $txt, $border=0, $align='J', $fill=false, $ln=1, $x=null, $y=null, $reseth=true, $stretch=0, $ishtml=false, $autopadding=true, $maxh=0, $valign='T', $fitcell=false) {
		...
		$this->setCellMargins(0, 0, 0, 0);
		if (WPO_WCPDF_IPS_PRO_TCPDF_STATIC::empty_string($this->lasth) OR $reseth) {
			// reset row height
			$this->resetLastH();
		}
		if (!WPO_WCPDF_IPS_PRO_TCPDF_STATIC::empty_string($y)) {
		...
		if (!WPO_WCPDF_IPS_PRO_TCPDF_STATIC::empty_string($x)) {
			$this->setX($x);
		} else {
			$x = $this->GetX();
		}
		...
		// get border modes
		$border_start = WPO_WCPDF_IPS_PRO_TCPDF_STATIC::getBorderMode($border, $position='start', $this->opencell);
		$border_end = WPO_WCPDF_IPS_PRO_TCPDF_STATIC::getBorderMode($border, $position='end', $this->opencell);
		$border_middle = WPO_WCPDF_IPS_PRO_TCPDF_STATIC::getBorderMode($border, $position='middle', $this->opencell);
	...
	public function getNumLines($txt, $w=0, $reseth=false, $autopadding=true, $cellpadding=null, $border=0) {
		...
		$lines = 1;
		$sum = 0;
		$chars = WPO_WCPDF_IPS_PRO_TCPDF_FONTS::utf8Bidi(WPO_WCPDF_IPS_PRO_TCPDF_FONTS::UTF8StringToArray($txt, $this->isunicode, $this->CurrentFont), $txt, $this->tmprtl, $this->isunicode, $this->CurrentFont);
		...
		for ($i = 0; $i < $length; ++$i) {
			...
			if (($c != 160)
					AND (($c == 173)
						OR preg_match($this->re_spaces, WPO_WCPDF_IPS_PRO_TCPDF_FONTS::unichr($c, $this->isunicode))
						OR (($c == 45)
							AND ($i > 0) AND ($i < ($length - 1))
							AND @preg_match('/[\p{L}]/'.$this->re_space['m'], WPO_WCPDF_IPS_PRO_TCPDF_FONTS::unichr($chars[($i - 1)], $this->isunicode))
							AND @preg_match('/[\p{L}]/'.$this->re_space['m'], WPO_WCPDF_IPS_PRO_TCPDF_FONTS::unichr($chars[($i + 1)], $this->isunicode))
						)
					)
				) {
	...
	public function Write($h, $txt, $link='', $fill=false, $align='', $ln=false, $stretch=0, $firstline=false, $firstblock=false, $maxh=0, $wadj=0, $margin=null) {
		...
		if (preg_match(WPO_WCPDF_IPS_PRO_TCPDF_FONT_DATA::$uni_RE_PATTERN_ARABIC, $s)) {
			$arabic = true;
		} else {
			$arabic = false;
		}
		// check if string contains RTL text
		if ($arabic OR ($this->tmprtl == 'R') OR preg_match(WPO_WCPDF_IPS_PRO_TCPDF_FONT_DATA::$uni_RE_PATTERN_RTL, $s)) {
			$rtlmode = true;
		} else {
			$rtlmode = false;
		}
		...
		$chars = WPO_WCPDF_IPS_PRO_TCPDF_FONTS::UTF8StringToArray($s, $this->isunicode, $this->CurrentFont);
		...
		// get array of chars
		$uchars = WPO_WCPDF_IPS_PRO_TCPDF_FONTS::UTF8ArrayToUniArray($chars, $this->isunicode);
		// get the number of characters
		$nb = count($chars);
		// replacement for SHY character (minus symbol)
		$shy_replacement = 45;
		$shy_replacement_char = WPO_WCPDF_IPS_PRO_TCPDF_FONTS::unichr($shy_replacement, $this->isunicode);
		...
		while ($i < $nb) {
			...
			if ($c == 10) { // 10 = "\n" = new line
				...
				$tmpstr = WPO_WCPDF_IPS_PRO_TCPDF_FONTS::UniArrSubString($uchars, $j, $i);
				if ($firstline) {
					$startx = $this->x;
					$tmparr = array_slice($chars, $j, ($i - $j));
					if ($rtlmode) {
						$tmparr = WPO_WCPDF_IPS_PRO_TCPDF_FONTS::utf8Bidi($tmparr, $tmpstr, $this->tmprtl, $this->isunicode, $this->CurrentFont);
					}
				...
				if ($firstline) {
					...
					return (WPO_WCPDF_IPS_PRO_TCPDF_FONTS::UniArrSubString($uchars, $i));
				}
				++$nl;
				$j = $i + 1;
				$l = 0;
				...
				if (($c != 160)
					AND (($c == 173)
						...
						OR (($c == 45)
							AND ($i < ($nb - 1))
							AND @preg_match('/[\p{L}]/'.$this->re_space['m'], WPO_WCPDF_IPS_PRO_TCPDF_FONTS::unichr($pc, $this->isunicode))
							AND @preg_match('/[\p{L}]/'.$this->re_space['m'], WPO_WCPDF_IPS_PRO_TCPDF_FONTS::unichr($chars[($i + 1)], $this->isunicode))
						)
					)
				) {
				...
				if ($this->isUnicodeFont() AND ($arabic)) {
					...
					// *** very slow ***
					$l = $this->GetArrStringWidth(WPO_WCPDF_IPS_PRO_TCPDF_FONTS::utf8Bidi(array_slice($chars, $j, ($i - $j)), '', $this->tmprtl, $this->isunicode, $this->CurrentFont));
				} else {
				...
				if (($l > $wmax) OR (($c == 173) AND (($l + $tmp_shy_replacement_width) >= $wmax))) {
					...
					if ($sep == -1) {
						...
						if (($this->rtl AND ($this->x <= ($this->w - $this->rMargin - $this->cell_padding['R'] - $margin['R'] - $chrwidth)))
							...
							$linebreak = true;
							if ($firstline) {
								return (WPO_WCPDF_IPS_PRO_TCPDF_FONTS::UniArrSubString($uchars, $j));
							}
						} else {
						...
						$strrest = WPO_WCPDF_IPS_PRO_TCPDF_FONTS::UniArrSubString($uchars, ($sep + $endspace));
						$nextstr = WPO_WCPDF_IPS_PRO_TCPDF_STATIC::pregSplit('/'.$this->re_space['p'].'/', $this->re_space['m'], $this->stringTrim($strrest));
						if (isset($nextstr[0]) AND ($this->GetStringWidth($nextstr[0]) > $pw)) {
							...
							$tmpstr = WPO_WCPDF_IPS_PRO_TCPDF_FONTS::UniArrSubString($uchars, $j, ($sep + $endspace));
							if ($firstline) {
								$startx = $this->x;
							...
							if ($firstline) {
								...
								$this->cell_padding = $tmpcellpadding;
								return (WPO_WCPDF_IPS_PRO_TCPDF_FONTS::UniArrSubString($uchars, ($sep + $endspace)));
							}
							$i = $sep;
							$sep = -1;
							$shy = false;
		...
		if ($l > 0) {
			...
			$tmpstr = WPO_WCPDF_IPS_PRO_TCPDF_FONTS::UniArrSubString($uchars, $j, $nb);
			if ($firstline) {
				$startx = $this->x;
			...
			if ($firstline) {
				$this->cell_padding = $tmpcellpadding;
				return (WPO_WCPDF_IPS_PRO_TCPDF_FONTS::UniArrSubString($uchars, $nb));
			}
			++$nl;
		}
		if ($firstline) {
	...
	public function Image($file, $x=null, $y=null, $w=0, $h=0, $type='', $link='', $align='', $resize=false, $dpi=300, $palign='', $ismask=false, $imgmask=false, $border=0, $fitbox=false, $hidden=false, $fitonpage=false, $alt=false, $altimgs=array()) {
		...
		if (!empty($imgdata)) {
			// copy image to cache
			$original_file = $file;
			$file = WPO_WCPDF_IPS_PRO_TCPDF_STATIC::getObjFilename('img', $this->file_id);
			$fp = WPO_WCPDF_IPS_PRO_TCPDF_STATIC::fopenLocal($file, 'w');
			if (!$fp) {
		...
		if ($newimage) {
			...
			if ($type == '') {
				$type = WPO_WCPDF_IPS_PRO_TCPDF_IMAGES::getImageFileType($file, $imsize);
			} elseif ($type == 'jpg') {
				$type = 'jpeg';
			}
			// Specific image handlers (defined on WPO_WCPDF_IPS_PRO_TCPDF_IMAGES CLASS)
			$mtd = '_parse'.$type;
			// GD image handler function
			$gdfunction = 'imagecreatefrom'.$type;
			$info = false;
			if ((method_exists('WPO_WCPDF_IPS_PRO_TCPDF_IMAGES', $mtd)) AND (!($resize AND (function_exists($gdfunction) OR extension_loaded('imagick'))))) {
				// WPO_WCPDF_IPS_PRO_TCPDF image functions
				$info = WPO_WCPDF_IPS_PRO_TCPDF_IMAGES::$mtd($file);
					...
					if ($img !== false) {
						if ($resize) {
							...
							if (($type == 'gif') OR ($type == 'png')) {
								$imgr = WPO_WCPDF_IPS_PRO_TCPDF_IMAGES::setGDImageTransparency($imgr, $img);
							}
							imagecopyresampled($imgr, $img, 0, 0, 0, 0, $neww, $newh, $pixw, $pixh);
							$img = $imgr;
						}
						if (($type == 'gif') OR ($type == 'png')) {
							$info = WPO_WCPDF_IPS_PRO_TCPDF_IMAGES::_toPNG($img, WPO_WCPDF_IPS_PRO_TCPDF_STATIC::getObjFilename('img', $this->file_id));
						} else {
							$info = WPO_WCPDF_IPS_PRO_TCPDF_IMAGES::_toJPEG($img, $this->jpeg_quality, WPO_WCPDF_IPS_PRO_TCPDF_STATIC::getObjFilename('img', $this->file_id));
						}
					}
				} catch(Exception $e) {
					$info = false;
			...
			if (($info === false) AND extension_loaded('imagick')) {
				try {
					...
					$tempname = WPO_WCPDF_IPS_PRO_TCPDF_STATIC::getObjFilename('img', $this->file_id);
					$img->writeImage($tempname);
					$info = WPO_WCPDF_IPS_PRO_TCPDF_IMAGES::_parsejpeg($tempname);
					unlink($tempname);
					$img->destroy();
				} catch(Exception $e) {
					$info = false;
	...
	protected function ImagePngAlpha($file, $x, $y, $wpx, $hpx, $w, $h, $type, $link, $align, $resize, $dpi, $palign, $filehash='') {
		...
		if (($parsed === false) AND extension_loaded('imagick')) {
			try {
				...
				$img->readImage($file);
				// clone image object
				$imga = WPO_WCPDF_IPS_PRO_TCPDF_STATIC::objclone($img);
		...
		if ($parsed === false) {
			if (empty($parse_error)) {
				$this->Error('WPO_WCPDF_IPS_PRO_TCPDF requires the Imagick or GD extension to handle PNG images with alpha channel.');
			} else {
	...
	public function Ln($h=null, $cell=false) {
		...
		if (WPO_WCPDF_IPS_PRO_TCPDF_STATIC::empty_string($h)) {
			$h = $this->lasth;
		}
		$this->y += $h;
		$this->newline = true;
	...
	public function Output($name='doc.pdf', $dest='I') {
		...
		if ($this->sign) {
			...
			// remove filler space
			$byterange_string_len = strlen(WPO_WCPDF_IPS_PRO_TCPDF_STATIC::$byterange_string);
			// define the ByteRange
			$byte_range = array();
			$byte_range[0] = 0;
			$byte_range[1] = strpos($pdfdoc, WPO_WCPDF_IPS_PRO_TCPDF_STATIC::$byterange_string) + $byterange_string_len + 10;
			...
			$pdfdoc = str_replace(WPO_WCPDF_IPS_PRO_TCPDF_STATIC::$byterange_string, $byterange, $pdfdoc);
			// write the document to a temporary folder
			$tempdoc = WPO_WCPDF_IPS_PRO_TCPDF_STATIC::getObjFilename('doc', $this->file_id);
			$f = WPO_WCPDF_IPS_PRO_TCPDF_STATIC::fopenLocal($tempdoc, 'wb');
			if (!$f) {
			...
			$tempsign = WPO_WCPDF_IPS_PRO_TCPDF_STATIC::getObjFilename('sig', $this->file_id);
		...
		switch($dest) {
			case 'I': {
				...
				if (php_sapi_name() != 'cli') {
					...
					WPO_WCPDF_IPS_PRO_TCPDF_STATIC::sendOutputData($this->getBuffer(), $this->bufferlen);
				} else {
					echo $this->getBuffer();
				}
				break;
			...
			case 'FD': {
				...
				$f = WPO_WCPDF_IPS_PRO_TCPDF_STATIC::fopenLocal($name, 'wb');
				if (!$f) {
				...
				if ($dest == 'FI') {
					...
					WPO_WCPDF_IPS_PRO_TCPDF_STATIC::sendOutputData(file_get_contents($name), filesize($name));
				} elseif ($dest == 'FD') {
					// send headers to browser
					if (ob_get_contents()) {
						$this->Error('Some data has already been output, can\'t send PDF file');
	...
	/**
	 * Unset all class WPO_WCPDF_IPS_PRO_variables except the following critical WPO_WCPDF_IPS_PRO_variables.
	 * @param boolean $destroyall if true destroys all class WPO_WCPDF_IPS_PRO_variables, otherwise preserves critical WPO_WCPDF_IPS_PRO_variables.
		...
		if ($destroyall AND !$preserve_objcopy && isset($this->file_id)) {
			...
			if (isset($this->imagekeys)) {
				foreach($this->imagekeys as $file) {
					if (strpos($file, K_PATH_CACHE) === 0 && WPO_WCPDF_IPS_PRO_TCPDF_STATIC::file_exists($file)) {
						@unlink($file);
					}
				}
			}
	...
	protected function getInternalPageNumberAliases($a= '') {
		...
		$u = '{'.$a.'}';
		$alias['u'][] = WPO_WCPDF_IPS_PRO_TCPDF_STATIC::_escape($u);
		if ($this->isunicode) {
			$alias['u'][] = WPO_WCPDF_IPS_PRO_TCPDF_STATIC::_escape(WPO_WCPDF_IPS_PRO_TCPDF_FONTS::UTF8ToLatin1($u, $this->isunicode, $this->CurrentFont));
			$alias['u'][] = WPO_WCPDF_IPS_PRO_TCPDF_STATIC::_escape(WPO_WCPDF_IPS_PRO_TCPDF_FONTS::utf8StrRev($u, false, $this->tmprtl, $this->isunicode, $this->CurrentFont));
			$alias['a'][] = WPO_WCPDF_IPS_PRO_TCPDF_STATIC::_escape(WPO_WCPDF_IPS_PRO_TCPDF_FONTS::UTF8ToLatin1($a, $this->isunicode, $this->CurrentFont));
			$alias['a'][] = WPO_WCPDF_IPS_PRO_TCPDF_STATIC::_escape(WPO_WCPDF_IPS_PRO_TCPDF_FONTS::utf8StrRev($a, false, $this->tmprtl, $this->isunicode, $this->CurrentFont));
		}
		$alias['a'][] = WPO_WCPDF_IPS_PRO_TCPDF_STATIC::_escape($a);
		return $alias;
	}
	...
	protected function getAllInternalPageNumberAliases() {
		$basic_alias = array(WPO_WCPDF_IPS_PRO_TCPDF_STATIC::$alias_tot_pages, WPO_WCPDF_IPS_PRO_TCPDF_STATIC::$alias_num_page, WPO_WCPDF_IPS_PRO_TCPDF_STATIC::$alias_group_tot_pages, WPO_WCPDF_IPS_PRO_TCPDF_STATIC::$alias_group_num_page, WPO_WCPDF_IPS_PRO_TCPDF_STATIC::$alias_right_shift);
		$pnalias = array();
			...
			foreach ($alias as $a) {
				...
				if (($pos = strpos($page, $a)) !== false) {
					...
					if ($type == 'u') {
						...
						$shift = WPO_WCPDF_IPS_PRO_TCPDF_FONTS::UTF8ToUTF16BE($shift, false, $this->isunicode, $this->CurrentFont);
					} else {
	...
	protected function setPageBoxTypes($boxes) {
		...
		foreach ($boxes as $box) {
			if (in_array($box, WPO_WCPDF_IPS_PRO_TCPDF_STATIC::$pageboxes)) {
				$this->page_boxes[] = $box;
			}
		}
	}
	...
	protected function _putpages() {
		...
		$ptpa = WPO_WCPDF_IPS_PRO_TCPDF_STATIC::formatPageNumber(($this->starting_page_number + $num_pages - 1));
		$ptpu = WPO_WCPDF_IPS_PRO_TCPDF_FONTS::UTF8ToUTF16BE($ptpa, false, $this->isunicode, $this->CurrentFont);
		...
		for ($n = 1; $n <= $num_pages; ++$n) {
			...
			$pnpa = WPO_WCPDF_IPS_PRO_TCPDF_STATIC::formatPageNumber(($this->starting_page_number + $n - 1));
			$pnpu = WPO_WCPDF_IPS_PRO_TCPDF_FONTS::UTF8ToUTF16BE($pnpa, false, $this->isunicode, $this->CurrentFont);
			...
			if (!empty($this->pagegroups)) {
				if (isset($this->newpagegroup[$n])) {
					$pagegroupnum = 0;
					++$groupnum;
					$ptga = WPO_WCPDF_IPS_PRO_TCPDF_STATIC::formatPageNumber($this->pagegroups[$groupnum]);
					$ptgu = WPO_WCPDF_IPS_PRO_TCPDF_FONTS::UTF8ToUTF16BE($ptga, false, $this->isunicode, $this->CurrentFont);
					$ptg_num_chars = $this->GetNumChars($ptga);
				}
				++$pagegroupnum;
				$pnga = WPO_WCPDF_IPS_PRO_TCPDF_STATIC::formatPageNumber($pagegroupnum);
				$pngu = WPO_WCPDF_IPS_PRO_TCPDF_FONTS::UTF8ToUTF16BE($pnga, false, $this->isunicode, $this->CurrentFont);
				...
				list($temppage, $gdiff) = WPO_WCPDF_IPS_PRO_TCPDF_STATIC::replacePageNumAliases($temppage, $replace, $gdiff);
			}
			// replace page numbers
			$replace = array();
			...
			list($temppage, $pdiff) = WPO_WCPDF_IPS_PRO_TCPDF_STATIC::replacePageNumAliases($temppage, $replace, $pdiff);
			...
			if (isset($this->PageAnnots[$n])) {
				...
				foreach ($this->PageAnnots[$n] as $key => $pl) {
					...
					if (isset($pl['opt']['c']) AND (is_array($pl['opt']['c'])) AND !empty($pl['opt']['c'])) {
						$annots .= ' /C '.WPO_WCPDF_IPS_PRO_TCPDF_COLORS::getColorStringFromArray($pl['opt']['c']);
					}
					//$annots .= ' /StructParent ';
					//$annots .= ' /OC ';
						...
						case 'link': {
							if (is_string($pl['txt']) && !empty($pl['txt'])) {
								if ($pl['txt'][0] == '#') {
									...
									$annots .= ' /A <</S /GoTo /D /'.WPO_WCPDF_IPS_PRO_TCPDF_STATIC::encodeNameObject(substr($pl['txt'], 1)).'>>';
								} elseif ($pl['txt'][0] == '%') {
									// embedded PDF file
						...
						case 'widget': {
							...
							if (isset($pl['opt']['mk']) AND (is_array($pl['opt']['mk'])) AND !empty($pl['opt']['mk'])) {
								...
								if (isset($pl['opt']['mk']['bc']) AND (is_array($pl['opt']['mk']['bc']))) {
									$annots .= ' /BC '.WPO_WCPDF_IPS_PRO_TCPDF_COLORS::getColorStringFromArray($pl['opt']['mk']['bc']);
								}
								if (isset($pl['opt']['mk']['bg']) AND (is_array($pl['opt']['mk']['bg']))) {
									$annots .= ' /BG '.WPO_WCPDF_IPS_PRO_TCPDF_COLORS::getColorStringFromArray($pl['opt']['mk']['bg']);
								}
	...
	protected function _putfonts() {
		...
		foreach ($this->FontFiles as $file => $info) {
			...
			$fontfile = WPO_WCPDF_IPS_PRO_TCPDF_FONTS::getFontFullPath($file, $info['fontdir']);
			if (!WPO_WCPDF_IPS_PRO_TCPDF_STATIC::empty_string($fontfile)) {
				...
				if ((!$compressed) AND (isset($info['length2']))) {
					...
					$font = WPO_WCPDF_IPS_PRO_TCPDF_FONTS::_getTrueTypeFontSubset($font, $subsetchars);
		...
		foreach ($this->fontkeys as $k) {
			...
			if ($type == 'core') {
				...
				if (!WPO_WCPDF_IPS_PRO_TCPDF_STATIC::empty_string($font['file'])) {
	...
	protected function _puttruetypeunicode($font) {
		...
		$stream = WPO_WCPDF_IPS_PRO_TCPDF_FONT_DATA::$uni_identity_h;
		// ToUnicode Object
		$this->_newobj();
		...
		$out .= "\n".WPO_WCPDF_IPS_PRO_TCPDF_FONTS::_putfontwidths($font, 0);
		if (isset($font['ctg']) AND (!WPO_WCPDF_IPS_PRO_TCPDF_STATIC::empty_string($font['ctg']))) {
		...
		if (isset($font['ctg']) AND (!WPO_WCPDF_IPS_PRO_TCPDF_STATIC::empty_string($font['ctg']))) {
			...
			// search and get ctg font file to embedd
			$fontfile = WPO_WCPDF_IPS_PRO_TCPDF_FONTS::getFontFullPath($ctgfile, $fontdir);
			if (WPO_WCPDF_IPS_PRO_TCPDF_STATIC::empty_string($fontfile)) {
	...
	protected function _putcidfont0($font) {
		...
		$out .= ' /FontDescriptor '.($this->n + 1).' 0 R';
		$out .= ' /DW '.$font['dw'];
		$out .= "\n".WPO_WCPDF_IPS_PRO_TCPDF_FONTS::_putfontwidths($font, $cidoffset);
		$out .= ' >>';
		$out .= "\n".'endobj';
		$this->_out($out);
		$this->_newobj();
	...
	protected function _putinfo() {
		...
		if (!WPO_WCPDF_IPS_PRO_TCPDF_STATIC::empty_string($this->title)) {
			// The document's title.
			$out .= ' /Title '.$this->_textstring($this->title, $oid);
		}
		if (!WPO_WCPDF_IPS_PRO_TCPDF_STATIC::empty_string($this->author)) {
			// The name of the person who created the document.
			$out .= ' /Author '.$this->_textstring($this->author, $oid);
		}
		if (!WPO_WCPDF_IPS_PRO_TCPDF_STATIC::empty_string($this->subject)) {
			// The subject of the document.
			$out .= ' /Subject '.$this->_textstring($this->subject, $oid);
		}
		if (!WPO_WCPDF_IPS_PRO_TCPDF_STATIC::empty_string($this->keywords)) {
			// Keywords associated with the document.
			$out .= ' /Keywords '.$this->_textstring($this->keywords, $oid);
		}
		if (!WPO_WCPDF_IPS_PRO_TCPDF_STATIC::empty_string($this->creator)) {
		...
		// default producer
		$out .= ' /Producer '.$this->_textstring(WPO_WCPDF_IPS_PRO_TCPDF_STATIC::getTCPDFProducer(), $oid);
	...
	protected function _putXMP() {
		...
		$this->encrypted = false;
		// set XMP data
		$xmp = '<?xpacket begin="'.WPO_WCPDF_IPS_PRO_TCPDF_FONTS::unichr(0xfeff, $this->isunicode).'" id="W5M0MpCehiHzreSzNTczkc9d"?>'."\n";
		...
		$xmp .= "\t\t\t".'<dc:title>'."\n";
		$xmp .= "\t\t\t\t".'<rdf:Alt>'."\n";
		$xmp .= "\t\t\t\t\t".'<rdf:li xml:lang="x-default">'.WPO_WCPDF_IPS_PRO_TCPDF_STATIC::_escapeXML($this->title).'</rdf:li>'."\n";
		$xmp .= "\t\t\t\t".'</rdf:Alt>'."\n";
		$xmp .= "\t\t\t".'</dc:title>'."\n";
		$xmp .= "\t\t\t".'<dc:creator>'."\n";
		$xmp .= "\t\t\t\t".'<rdf:Seq>'."\n";
		$xmp .= "\t\t\t\t\t".'<rdf:li>'.WPO_WCPDF_IPS_PRO_TCPDF_STATIC::_escapeXML($this->author).'</rdf:li>'."\n";
		$xmp .= "\t\t\t\t".'</rdf:Seq>'."\n";
		$xmp .= "\t\t\t".'</dc:creator>'."\n";
		$xmp .= "\t\t\t".'<dc:description>'."\n";
		$xmp .= "\t\t\t\t".'<rdf:Alt>'."\n";
		$xmp .= "\t\t\t\t\t".'<rdf:li xml:lang="x-default">'.WPO_WCPDF_IPS_PRO_TCPDF_STATIC::_escapeXML($this->subject).'</rdf:li>'."\n";
		$xmp .= "\t\t\t\t".'</rdf:Alt>'."\n";
		$xmp .= "\t\t\t".'</dc:description>'."\n";
		$xmp .= "\t\t\t".'<dc:subject>'."\n";
		$xmp .= "\t\t\t\t".'<rdf:Bag>'."\n";
		$xmp .= "\t\t\t\t\t".'<rdf:li>'.WPO_WCPDF_IPS_PRO_TCPDF_STATIC::_escapeXML($this->keywords).'</rdf:li>'."\n";
		$xmp .= "\t\t\t\t".'</rdf:Bag>'."\n";
		$xmp .= "\t\t\t".'</dc:subject>'."\n";
		$xmp .= "\t\t".'</rdf:Description>'."\n";
		// convert doc creation date format
		$dcdate = WPO_WCPDF_IPS_PRO_TCPDF_STATIC::getFormattedDate($this->doc_creation_timestamp);
		...
		$doccreationdate = WPO_WCPDF_IPS_PRO_TCPDF_STATIC::_escapeXML($doccreationdate);
		// convert doc modification date format
		$dmdate = WPO_WCPDF_IPS_PRO_TCPDF_STATIC::getFormattedDate($this->doc_modification_timestamp);
		...
		$docmoddate = WPO_WCPDF_IPS_PRO_TCPDF_STATIC::_escapeXML($docmoddate);
		...
		$xmp .= "\t\t\t".'<pdf:Keywords>'.WPO_WCPDF_IPS_PRO_TCPDF_STATIC::_escapeXML($this->keywords).'</pdf:Keywords>'."\n";
		$xmp .= "\t\t\t".'<pdf:Producer>'.WPO_WCPDF_IPS_PRO_TCPDF_STATIC::_escapeXML(WPO_WCPDF_IPS_PRO_TCPDF_STATIC::getTCPDFProducer()).'</pdf:Producer>'."\n";
		$xmp .= "\t\t".'</rdf:Description>'."\n";
	...
	protected function _putcatalog() {
		...
		if (isset($this->LayoutMode) AND (!WPO_WCPDF_IPS_PRO_TCPDF_STATIC::empty_string($this->LayoutMode))) {
			$out .= ' /PageLayout /'.$this->LayoutMode;
		}
		if (isset($this->PageMode) AND (!WPO_WCPDF_IPS_PRO_TCPDF_STATIC::empty_string($this->PageMode))) {
		...
		if (!empty($this->pdflayers)) {
			...
			$out .= ' /Creator '.$this->_textstring('WPO_WCPDF_IPS_PRO_TCPDF', $oid);
			$out .= ' /BaseState /ON';
			$out .= ' /OFF ['.$lyrobjs_off.']';
	...
	protected function _putviewerpreferences() {
		...
		if (isset($vp['Duplex']) AND (!WPO_WCPDF_IPS_PRO_TCPDF_STATIC::empty_string($vp['Duplex']))) {
	...
	protected function _beginpage($orientation='', $format='') {
		...
		$this->state = 2;
		if (WPO_WCPDF_IPS_PRO_TCPDF_STATIC::empty_string($orientation)) {
			if (isset($this->CurOrientation)) {
		...
		if (WPO_WCPDF_IPS_PRO_TCPDF_STATIC::empty_string($format)) {
	...
	protected function _getobj($objid=null) {
		if (WPO_WCPDF_IPS_PRO_TCPDF_STATIC::empty_string($objid)) {
			++$this->n;
			$objid = $this->n;
		}
	...
	protected function _datastring($s, $n=0) {
		...
		return '('. WPO_WCPDF_IPS_PRO_TCPDF_STATIC::_escape($s).')';
	}
	...
	public function setDocCreationTimestamp($time) {
		if (is_string($time)) {
			$time = WPO_WCPDF_IPS_PRO_TCPDF_STATIC::getTimestamp($time);
		}
	...
	protected function _datestring($n=0, $timestamp=0) {
		...
		return $this->_datastring('D:'.WPO_WCPDF_IPS_PRO_TCPDF_STATIC::getFormattedDate($timestamp), $n);
	}
	...
	protected function _textstring($s, $n=0) {
		if ($this->isunicode) {
			...
			$s = WPO_WCPDF_IPS_PRO_TCPDF_FONTS::UTF8ToUTF16BE($s, true, $this->isunicode, $this->CurrentFont);
		}
	...
	protected function _objectkey($n) {
		...
		$objkey = substr(WPO_WCPDF_IPS_PRO_TCPDF_STATIC::_md5_16($objkey), 0, (($this->encryptdata['Length'] / 8) + 5));
	...
	protected function _encrypt_data($n, $s) {
		...
		switch ($this->encryptdata['mode']) {
			case 0:   // RC4-40
			case 1: { // RC4-128
				$s = WPO_WCPDF_IPS_PRO_TCPDF_STATIC::_RC4($this->_objectkey($n), $s, $this->last_enc_key, $this->last_enc_key_c);
				break;
			}
			case 2: { // AES-128
				$s = WPO_WCPDF_IPS_PRO_TCPDF_STATIC::_AES($this->_objectkey($n), $s);
				break;
			}
			case 3: { // AES-256
				$s = WPO_WCPDF_IPS_PRO_TCPDF_STATIC::_AES($this->encryptdata['key'], $s);
				break;
			}
		}
		return $s;
	...
	protected function _putencryption() {
		...
		if ($this->encryptdata['pubkey']) {
			...
			if ($this->encryptdata['V'] == 5) { // AES-256
				$out .= ' 5';
				$out .= ' /OE ('.WPO_WCPDF_IPS_PRO_TCPDF_STATIC::_escape($this->encryptdata['OE']).')';
				$out .= ' /UE ('.WPO_WCPDF_IPS_PRO_TCPDF_STATIC::_escape($this->encryptdata['UE']).')';
				$out .= ' /Perms ('.WPO_WCPDF_IPS_PRO_TCPDF_STATIC::_escape($this->encryptdata['perms']).')';
			...
			$out .= ' /O ('.WPO_WCPDF_IPS_PRO_TCPDF_STATIC::_escape($this->encryptdata['O']).')';
			$out .= ' /U ('.WPO_WCPDF_IPS_PRO_TCPDF_STATIC::_escape($this->encryptdata['U']).')';
	...
	protected function _Uvalue() {
		if ($this->encryptdata['mode'] == 0) { // RC4-40
			return WPO_WCPDF_IPS_PRO_TCPDF_STATIC::_RC4($this->encryptdata['key'], WPO_WCPDF_IPS_PRO_TCPDF_STATIC::$enc_padding, $this->last_enc_key, $this->last_enc_key_c);
		} elseif ($this->encryptdata['mode'] < 3) { // RC4-128, AES-128
			$tmp = WPO_WCPDF_IPS_PRO_TCPDF_STATIC::_md5_16(WPO_WCPDF_IPS_PRO_TCPDF_STATIC::$enc_padding.$this->encryptdata['fileid']);
			$enc = WPO_WCPDF_IPS_PRO_TCPDF_STATIC::_RC4($this->encryptdata['key'], $tmp, $this->last_enc_key, $this->last_enc_key_c);
			$len = strlen($tmp);
			for ($i = 1; $i <= 19; ++$i) {
				$ek = '';
				for ($j = 0; $j < $len; ++$j) {
				...
				$enc = WPO_WCPDF_IPS_PRO_TCPDF_STATIC::_RC4($ek, $enc, $this->last_enc_key, $this->last_enc_key_c);
			}
			$enc .= str_repeat("\x00", 16);
			return substr($enc, 0, 32);
		} elseif ($this->encryptdata['mode'] == 3) { // AES-256
			$seed = WPO_WCPDF_IPS_PRO_TCPDF_STATIC::_md5_16(WPO_WCPDF_IPS_PRO_TCPDF_STATIC::getRandomSeed());
	...
	protected function _UEvalue() {
		...
		return WPO_WCPDF_IPS_PRO_TCPDF_STATIC::_AESnopad($hashkey, $this->encryptdata['key']);
	}
	...
	protected function _Ovalue() {
		if ($this->encryptdata['mode'] < 3) { // RC4-40, RC4-128, AES-128
			$tmp = WPO_WCPDF_IPS_PRO_TCPDF_STATIC::_md5_16($this->encryptdata['owner_password']);
			if ($this->encryptdata['mode'] > 0) {
				for ($i = 0; $i < 50; ++$i) {
					$tmp = WPO_WCPDF_IPS_PRO_TCPDF_STATIC::_md5_16($tmp);
				}
			}
			$owner_key = substr($tmp, 0, ($this->encryptdata['Length'] / 8));
			$enc = WPO_WCPDF_IPS_PRO_TCPDF_STATIC::_RC4($owner_key, $this->encryptdata['user_password'], $this->last_enc_key, $this->last_enc_key_c);
			if ($this->encryptdata['mode'] > 0) {
	...
	protected function _fixAES256Password($password) {
		...
		$psw_array = WPO_WCPDF_IPS_PRO_TCPDF_FONTS::utf8Bidi(WPO_WCPDF_IPS_PRO_TCPDF_FONTS::UTF8StringToArray($password, $this->isunicode, $this->CurrentFont), $password, $this->rtl, $this->isunicode, $this->CurrentFont);
		foreach ($psw_array as $c) {
			$psw .= WPO_WCPDF_IPS_PRO_TCPDF_FONTS::unichr($c, $this->isunicode);
		}
	...
	protected function _generateencryptionkey() {
		...
		if (!$this->encryptdata['pubkey']) { // standard mode
			if ($this->encryptdata['mode'] == 3) { // AES-256
				...
				$this->encryptdata['key'] = substr(hash('sha256', WPO_WCPDF_IPS_PRO_TCPDF_STATIC::getRandomSeed(), true), 0, $keybytelen);
				// truncate passwords
				...
				$perms = WPO_WCPDF_IPS_PRO_TCPDF_STATIC::getEncPermissionsString($this->encryptdata['protection']); // bytes 0-3
				...
				$this->encryptdata['perms'] = WPO_WCPDF_IPS_PRO_TCPDF_STATIC::_AESnopad($this->encryptdata['key'], $perms);
			} else { // RC4-40, RC4-128, AES-128
				// Pad passwords
				$this->encryptdata['user_password'] = substr($this->encryptdata['user_password'].WPO_WCPDF_IPS_PRO_TCPDF_STATIC::$enc_padding, 0, 32);
				$this->encryptdata['owner_password'] = substr($this->encryptdata['owner_password'].WPO_WCPDF_IPS_PRO_TCPDF_STATIC::$enc_padding, 0, 32);
				// Compute O value
				$this->encryptdata['O'] = $this->_Ovalue();
				// get default permissions (reverse byte order)
				$permissions = WPO_WCPDF_IPS_PRO_TCPDF_STATIC::getEncPermissionsString($this->encryptdata['protection']);
				// Compute encryption key
				$tmp = WPO_WCPDF_IPS_PRO_TCPDF_STATIC::_md5_16($this->encryptdata['user_password'].$this->encryptdata['O'].$permissions.$this->encryptdata['fileid']);
				if ($this->encryptdata['mode'] > 0) {
					for ($i = 0; $i < 50; ++$i) {
						$tmp = WPO_WCPDF_IPS_PRO_TCPDF_STATIC::_md5_16(substr($tmp, 0, $keybytelen));
					}
				}
			...
			// random 20-byte seed
			$seed = sha1(WPO_WCPDF_IPS_PRO_TCPDF_STATIC::getRandomSeed(), true);
			$recipient_bytes = '';
			foreach ($this->encryptdata['pubkeys'] as $pubkey) {
				// for each public certificate
				if (isset($pubkey['p'])) {
					$pkprotection = WPO_WCPDF_IPS_PRO_TCPDF_STATIC::getUserPermissionCode($pubkey['p'], $this->encryptdata['mode']);
				} else {
					$pkprotection = $this->encryptdata['protection'];
				}
				// get default permissions (reverse byte order)
				$pkpermissions = WPO_WCPDF_IPS_PRO_TCPDF_STATIC::getEncPermissionsString($pkprotection);
				// envelope data
				$envelope = $seed.$pkpermissions;
				// write the envelope data to a temporary file
				$tempkeyfile = WPO_WCPDF_IPS_PRO_TCPDF_STATIC::getObjFilename('key', $this->file_id);
				$f = WPO_WCPDF_IPS_PRO_TCPDF_STATIC::fopenLocal($tempkeyfile, 'wb');
				if (!$f) {
				...
				fclose($f);
				$tempencfile = WPO_WCPDF_IPS_PRO_TCPDF_STATIC::getObjFilename('enc', $this->file_id);
	...
	public function setProtection($permissions=array('print', 'modify', 'copy', 'annot-forms', 'fill-forms', 'extract', 'assemble', 'print-high'), $user_pass='', $owner_pass=null, $mode=0, $pubkeys=null) {
		...
		$this->encryptdata['protection'] = WPO_WCPDF_IPS_PRO_TCPDF_STATIC::getUserPermissionCode($permissions, $mode);
		...
		if ($owner_pass === null) {
			$owner_pass = md5(WPO_WCPDF_IPS_PRO_TCPDF_STATIC::getRandomSeed());
		}
		...
		$this->encrypted = true;
		$this->encryptdata['fileid'] = WPO_WCPDF_IPS_PRO_TCPDF_STATIC::convertHexStringToString($this->file_id);
	...
	// The following section is based on the code WPO_WCPDF_IPS_PRO_provided by David Hernandez Sanz
	...
	public function Rect($x, $y, $w, $h, $style='', $border_style=array(), $fill_color=array()) {
		...
		if (!empty($style)) {
			$op = WPO_WCPDF_IPS_PRO_TCPDF_STATIC::getPathPaintOperator($style);
	...
	public function Ellipse($x0, $y0, $rx, $ry=0, $angle=0, $astart=0, $afinish=360, $style='', $line_style=array(), $fill_color=array(), $nc=2) {
		...
		if (WPO_WCPDF_IPS_PRO_TCPDF_STATIC::empty_string($ry) OR ($ry == 0)) {
			$ry = $rx;
		}
		if (!(false === strpos($style, 'F')) AND is_array($fill_color)) {
			$this->setFillColorArray($fill_color);
	...
	public function setDestination($name, $y=-1, $page='', $x=-1) {
		...
		$name = WPO_WCPDF_IPS_PRO_TCPDF_STATIC::encodeNameObject($name);
		if (WPO_WCPDF_IPS_PRO_TCPDF_STATIC::empty_string($name)) {
			return false;
		}
		if ($y == -1) {
			$y = $this->GetY();
		...
		foreach ($this->outlines as $i => $o) {
			...
			if (isset($o['u']) AND !empty($o['u'])) {
				...
				if (is_string($o['u'])) {
					if ($o['u'][0] == '#') {
						// internal destination
						$out .= ' /Dest /'.WPO_WCPDF_IPS_PRO_TCPDF_STATIC::encodeNameObject(substr($o['u'], 1));
					} elseif ($o['u'][0] == '%') {
						// embedded PDF file
	...
	protected function _addfield($type, $name, $x, $y, $w, $h, $prop) {
		...
		foreach($prop as $key => $val) {
			if (strcmp(substr($key, -5), 'Color') == 0) {
				$val = WPO_WCPDF_IPS_PRO_TCPDF_COLORS::_JScolor($val);
			} else {
				$val = "'".$val."'";
			}
	...
	public function TextField($name, $w, $h, $prop=array(), $opt=array(), $x=null, $y=null, $js=false) {
		...
		// get annotation data
		$popt = WPO_WCPDF_IPS_PRO_TCPDF_STATIC::getAnnotOptFromJSProp($prop, $this->spot_colors, $this->rtl);
	...
	public function RadioButton($name, $w, $prop=array(), $opt=array(), $onvalue='On', $checked=false, $x=null, $y=null, $js=false) {
		...
		if (WPO_WCPDF_IPS_PRO_TCPDF_STATIC::empty_string($onvalue)) {
			$onvalue = 'On';
		}
		if ($checked) {
			$defval = $onvalue;
	...
	protected function _putsignature() {
		...
		$out .= ' '.WPO_WCPDF_IPS_PRO_TCPDF_STATIC::$byterange_string;
		...
		if (empty($this->signature_data['approval']) OR ($this->signature_data['approval'] != 'A')) {
			...
			if ($this->signature_data['cert_type'] > 0) {
				...
				$out .= ' /Type /TransformParams';
				$out .= ' /V /2.2';
				if (!WPO_WCPDF_IPS_PRO_TCPDF_STATIC::empty_string($this->ur['document'])) {
					$out .= ' /Document['.$this->ur['document'].']';
				}
				if (!WPO_WCPDF_IPS_PRO_TCPDF_STATIC::empty_string($this->ur['form'])) {
					$out .= ' /Form['.$this->ur['form'].']';
				}
				if (!WPO_WCPDF_IPS_PRO_TCPDF_STATIC::empty_string($this->ur['signature'])) {
					$out .= ' /Signature['.$this->ur['signature'].']';
				}
				if (!WPO_WCPDF_IPS_PRO_TCPDF_STATIC::empty_string($this->ur['annots'])) {
					$out .= ' /Annots['.$this->ur['annots'].']';
				}
				if (!WPO_WCPDF_IPS_PRO_TCPDF_STATIC::empty_string($this->ur['ef'])) {
					$out .= ' /EF['.$this->ur['ef'].']';
				}
				if (!WPO_WCPDF_IPS_PRO_TCPDF_STATIC::empty_string($this->ur['formex'])) {
		...
		if (isset($this->signature_data['info']['Name']) AND !WPO_WCPDF_IPS_PRO_TCPDF_STATIC::empty_string($this->signature_data['info']['Name'])) {
			$out .= ' /Name '.$this->_textstring($this->signature_data['info']['Name'], $sigobjid);
		}
		if (isset($this->signature_data['info']['Location']) AND !WPO_WCPDF_IPS_PRO_TCPDF_STATIC::empty_string($this->signature_data['info']['Location'])) {
			$out .= ' /Location '.$this->_textstring($this->signature_data['info']['Location'], $sigobjid);
		}
		if (isset($this->signature_data['info']['Reason']) AND !WPO_WCPDF_IPS_PRO_TCPDF_STATIC::empty_string($this->signature_data['info']['Reason'])) {
			$out .= ' /Reason '.$this->_textstring($this->signature_data['info']['Reason'], $sigobjid);
		}
		if (isset($this->signature_data['info']['ContactInfo']) AND !WPO_WCPDF_IPS_PRO_TCPDF_STATIC::empty_string($this->signature_data['info']['ContactInfo'])) {
	...
	public function getAliasRightShift() {
		...
		$ref = '{'.WPO_WCPDF_IPS_PRO_TCPDF_STATIC::$alias_right_shift.'}{'.WPO_WCPDF_IPS_PRO_TCPDF_STATIC::$alias_tot_pages.'}{'.WPO_WCPDF_IPS_PRO_TCPDF_STATIC::$alias_num_page.'}';
		...
		$alias = WPO_WCPDF_IPS_PRO_TCPDF_STATIC::$alias_right_shift.$sdiff.'}';
		if ($this->isUnicodeFont()) {
			$alias = '{'.$alias;
		}
		return $alias;
	...
	public function getAliasNbPages() {
		if ($this->isUnicodeFont()) {
			return '{'.WPO_WCPDF_IPS_PRO_TCPDF_STATIC::$alias_tot_pages.'}';
		}
		return WPO_WCPDF_IPS_PRO_TCPDF_STATIC::$alias_tot_pages;
	}
	...
	public function getAliasNumPage() {
		if ($this->isUnicodeFont()) {
			return '{'.WPO_WCPDF_IPS_PRO_TCPDF_STATIC::$alias_num_page.'}';
		}
		return WPO_WCPDF_IPS_PRO_TCPDF_STATIC::$alias_num_page;
	}
	...
	public function getPageGroupAlias() {
		if ($this->isUnicodeFont()) {
			return '{'.WPO_WCPDF_IPS_PRO_TCPDF_STATIC::$alias_group_tot_pages.'}';
		}
		return WPO_WCPDF_IPS_PRO_TCPDF_STATIC::$alias_group_tot_pages;
	}
	...
	public function getPageNumGroupAlias() {
		if ($this->isUnicodeFont()) {
			return '{'.WPO_WCPDF_IPS_PRO_TCPDF_STATIC::$alias_group_num_page.'}';
		}
		return WPO_WCPDF_IPS_PRO_TCPDF_STATIC::$alias_group_num_page;
	}
	...
	public function getGroupPageNoFormatted() {
		return WPO_WCPDF_IPS_PRO_TCPDF_STATIC::formatPageNumber($this->getGroupPageNo());
	}
	...
	public function PageNoFormatted() {
		return WPO_WCPDF_IPS_PRO_TCPDF_STATIC::formatPageNumber($this->PageNo());
	}
	...
	public function setOverprint($stroking=true, $nonstroking=null, $mode=0) {
		...
		if (WPO_WCPDF_IPS_PRO_TCPDF_STATIC::empty_string($nonstroking)) {
		...
		foreach ($bars as $col) {
			switch ($col) {
				...
				default: { // SPECIFIC SPOT COLOR NAME
					$col_a = array(0,0,0,0,'None');
					$col_b = WPO_WCPDF_IPS_PRO_TCPDF_COLORS::getSpotColor($col, $this->spot_colors);
					if ($col_b === false) {
	...
	public function write1DBarcode($code, $type, $x=null, $y=null, $w=null, $h=null, $xres=null, $style=array(), $align='') {
		if (WPO_WCPDF_IPS_PRO_TCPDF_STATIC::empty_string(trim($code))) {
			return;
		}
		...
		$barcodeobj = new WPO_WCPDF_IPS_PRO_TCPDFBarcode($code, $type);
		...
		if ($style['stretch']) {
			...
			if (WPO_WCPDF_IPS_PRO_TCPDF_STATIC::empty_string($xres)) {
		...
		// height
		if (WPO_WCPDF_IPS_PRO_TCPDF_STATIC::empty_string($h) OR ($h <= 0)) {
			// set default height
		...
		// print text
		if ($style['text']) {
			if (isset($style['label']) AND !WPO_WCPDF_IPS_PRO_TCPDF_STATIC::empty_string($style['label'])) {
				$label = $style['label'];
			} else {
				$label = $code;
			}
	...
	public function write2DBarcode($code, $type, $x=null, $y=null, $w=null, $h=null, $style=array(), $align='', $distort=false) {
		...
		$gvars = $this->getGraphicVars();
		// create new barcode object
		$barcodeobj = new WPO_WCPDF_IPS_PRO_TCPDF2DBarcode($code, $type);
		$arrcode = $barcodeobj->getBarcodeArray();
		...
		// set dimensions
		if ((WPO_WCPDF_IPS_PRO_TCPDF_STATIC::empty_string($w) OR ($w <= 0)) AND (WPO_WCPDF_IPS_PRO_TCPDF_STATIC::empty_string($h) OR ($h <= 0))) {
	...
	public function fixHTMLCode($html, $default_css='', $tagvs=null, $tidy_options=null) {
		return WPO_WCPDF_IPS_PRO_TCPDF_STATIC::fixHTMLCode($html, $default_css, $tagvs, $tidy_options, $this->tagvspaces);
	}
	...
	protected function getCSSBorderStyle($cssborder) {
		...
		$border['color'] = WPO_WCPDF_IPS_PRO_TCPDF_COLORS::convertHTMLColorToDec($color, $this->spot_colors);
		return $border;
	}
	...
	public function getHTMLFontUnits($val, $refsize=12, $parent_size=12, $defaultunit='pt') {
		$refsize = WPO_WCPDF_IPS_PRO_TCPDF_FONTS::getFontRefSize($refsize);
		$parent_size = WPO_WCPDF_IPS_PRO_TCPDF_FONTS::getFontRefSize($parent_size, $refsize);
		switch ($val) {
			case 'xx-small': {
				$size = ($refsize - 4);
				break;
					...
					if (empty($type) OR (isset($type[1]) AND (($type[1] == 'all') OR ($type[1] == 'print')))) {
						...
						if (preg_match('/href[\s]*=[\s]*"([^"]*)"/', $link, $type) > 0) {
							...
							if (($cssdata !== FALSE) AND (strlen($cssdata) > 0)) {
								$css = array_merge($css, WPO_WCPDF_IPS_PRO_TCPDF_STATIC::extractCSSproperties($cssdata));
							}
						}
					}
				}
			...
			if (preg_match($tagpattern, $element)) {
				...
				if ($element[0] == '/') {
					...
					if (
						...
						if (WPO_WCPDF_IPS_PRO_TCPDF_STATIC::empty_string($dom[($dom[($dom[$key]['parent'])]['parent'])]['thead'])) {
					...
					if (($dom[$key]['value'] == 'table') AND (!WPO_WCPDF_IPS_PRO_TCPDF_STATIC::empty_string($dom[($dom[$key]['parent'])]['thead']))) {
					...
					if (!empty($css)) {
						...
						list($dom[$key]['csssel'], $dom[$key]['cssdata']) = WPO_WCPDF_IPS_PRO_TCPDF_STATIC::getCSSdataArray($dom, $key, $css);
						$dom[$key]['attribute']['style'] = WPO_WCPDF_IPS_PRO_TCPDF_STATIC::getTagStyleFromCSSarray($dom[$key]['cssdata']);
					}
					...
					if (isset($dom[$key]['attribute']['style']) AND !empty($dom[$key]['attribute']['style'])) {
						...
						// font color
						if (isset($dom[$key]['style']['color']) AND (!WPO_WCPDF_IPS_PRO_TCPDF_STATIC::empty_string($dom[$key]['style']['color']))) {
							$dom[$key]['fgcolor'] = WPO_WCPDF_IPS_PRO_TCPDF_COLORS::convertHTMLColorToDec($dom[$key]['style']['color'], $this->spot_colors);
						} elseif ($dom[$key]['value'] == 'a') {
							$dom[$key]['fgcolor'] = $this->htmlLinkColorArray;
						}
						// background color
						if (isset($dom[$key]['style']['background-color']) AND (!WPO_WCPDF_IPS_PRO_TCPDF_STATIC::empty_string($dom[$key]['style']['background-color']))) {
							$dom[$key]['bgcolor'] = WPO_WCPDF_IPS_PRO_TCPDF_COLORS::convertHTMLColorToDec($dom[$key]['style']['background-color'], $this->spot_colors);
						}
						// text-decoration
						if (isset($dom[$key]['style']['text-decoration'])) {
							...
							foreach ($decors as $dec) {
								$dec = trim($dec);
								if (!WPO_WCPDF_IPS_PRO_TCPDF_STATIC::empty_string($dec)) {
									if ($dec[0] == 'u') {
										// underline
										$dom[$key]['fontstyle'] .= 'U';
									} elseif ($dec[0] == 'l') {
						...
						if (isset($dom[$key]['style']['border-color'])) {
							...
							if (isset($brd_colors[3])) {
								$dom[$key]['border']['L']['color'] = WPO_WCPDF_IPS_PRO_TCPDF_COLORS::convertHTMLColorToDec($brd_colors[3], $this->spot_colors);
							}
							if (isset($brd_colors[1])) {
								$dom[$key]['border']['R']['color'] = WPO_WCPDF_IPS_PRO_TCPDF_COLORS::convertHTMLColorToDec($brd_colors[1], $this->spot_colors);
							}
							if (isset($brd_colors[0])) {
								$dom[$key]['border']['T']['color'] = WPO_WCPDF_IPS_PRO_TCPDF_COLORS::convertHTMLColorToDec($brd_colors[0], $this->spot_colors);
							}
							if (isset($brd_colors[2])) {
								$dom[$key]['border']['B']['color'] = WPO_WCPDF_IPS_PRO_TCPDF_COLORS::convertHTMLColorToDec($brd_colors[2], $this->spot_colors);
							}
						}
						...
						foreach ($cellside as $bsk => $bsv) {
							...
							if (isset($dom[$key]['style']['border-'.$bsv.'-color'])) {
								$dom[$key]['border'][$bsk]['color'] = WPO_WCPDF_IPS_PRO_TCPDF_COLORS::convertHTMLColorToDec($dom[$key]['style']['border-'.$bsv.'-color'], $this->spot_colors);
							}
					...
					if ((($dom[$key]['value'] == 'ul') OR ($dom[$key]['value'] == 'ol') OR ($dom[$key]['value'] == 'dl'))
						AND (!isset($dom[$key]['align']) OR WPO_WCPDF_IPS_PRO_TCPDF_STATIC::empty_string($dom[$key]['align']) OR ($dom[$key]['align'] != 'J'))) {
						if ($this->rtl) {
							$dom[$key]['align'] = 'R';
						} else {
							$dom[$key]['align'] = 'L';
					...
					if (isset($dom[$key]['attribute']['color']) AND (!WPO_WCPDF_IPS_PRO_TCPDF_STATIC::empty_string($dom[$key]['attribute']['color']))) {
						$dom[$key]['fgcolor'] = WPO_WCPDF_IPS_PRO_TCPDF_COLORS::convertHTMLColorToDec($dom[$key]['attribute']['color'], $this->spot_colors);
					} elseif (!isset($dom[$key]['style']['color']) AND ($dom[$key]['value'] == 'a')) {
						$dom[$key]['fgcolor'] = $this->htmlLinkColorArray;
					}
					// set background color attribute
					if (isset($dom[$key]['attribute']['bgcolor']) AND (!WPO_WCPDF_IPS_PRO_TCPDF_STATIC::empty_string($dom[$key]['attribute']['bgcolor']))) {
						$dom[$key]['bgcolor'] = WPO_WCPDF_IPS_PRO_TCPDF_COLORS::convertHTMLColorToDec($dom[$key]['attribute']['bgcolor'], $this->spot_colors);
					}
					// set stroke color attribute
					if (isset($dom[$key]['attribute']['strokecolor']) AND (!WPO_WCPDF_IPS_PRO_TCPDF_STATIC::empty_string($dom[$key]['attribute']['strokecolor']))) {
						$dom[$key]['strokecolor'] = WPO_WCPDF_IPS_PRO_TCPDF_COLORS::convertHTMLColorToDec($dom[$key]['attribute']['strokecolor'], $this->spot_colors);
					}
					// check for width attribute
					...
					// check for text alignment
					if (isset($dom[$key]['attribute']['align']) AND (!WPO_WCPDF_IPS_PRO_TCPDF_STATIC::empty_string($dom[$key]['attribute']['align'])) AND ($dom[$key]['value'] !== 'img')) {
	...
	/**
	 * Serialize data to be used with WPO_WCPDF_IPS_PRO_TCPDF tag in HTML code.
	 * @param string $method WPO_WCPDF_IPS_PRO_TCPDF method name
	...
	/**
	 * Unserialize data to be used with WPO_WCPDF_IPS_PRO_TCPDF tag in HTML code.
	...
	/**
	 * Check if a WPO_WCPDF_IPS_PRO_TCPDF tag is allowed
	 * @param string $method WPO_WCPDF_IPS_PRO_TCPDF method name
	 * @return boolean
	 * @protected
	 */
	...
	public function writeHTML($html, $ln=true, $fill=false, $reseth=false, $cell=false, $align='') {
		...
		while ($key < $maxel) {
			...
			if (($dom[$key]['value'] == 'tr') AND isset($dom[$key]['thead']) AND $dom[$key]['thead']) {
				if (isset($dom[$key]['parent']) AND isset($dom[$dom[$key]['parent']]['thead']) AND !WPO_WCPDF_IPS_PRO_TCPDF_STATIC::empty_string($dom[$dom[$key]['parent']]['thead'])) {
					$this->inthead = true;
			...
			if ($dom[$key]['tag'] OR ($key == 0)) {
				...
				if (WPO_WCPDF_IPS_PRO_TCPDF_STATIC::empty_string($lalign)) {
					$lalign = $align;
				}
			}
			// align lines
				...
				if (isset($startlinex)) {
					...
					if ((((($plalign == 'C') OR ($plalign == 'J') OR (($plalign == 'R') AND (!$this->rtl)) OR (($plalign == 'L') AND ($this->rtl)))))) {
						...
						if ($this->isRTLTextDir()) { // RTL
							...
							$pos1 = WPO_WCPDF_IPS_PRO_TCPDF_STATIC::revstrpos($pmid, '[(');
							if ($pos1 > 0) {
								$pos1 = intval($pos1);
								if ($this->isUnicodeFont()) {
									$pos2 = intval(WPO_WCPDF_IPS_PRO_TCPDF_STATIC::revstrpos($pmid, '[('.chr(0).chr(32)));
									$spacelen = 2;
								} else {
									$pos2 = intval(WPO_WCPDF_IPS_PRO_TCPDF_STATIC::revstrpos($pmid, '[('.chr(32)));
									$spacelen = 1;
								}
								if ($pos1 == $pos2) {
							...
							if ($pos1 > 0) {
								$pos1 = intval($pos1);
								if ($this->isUnicodeFont()) {
									$pos2 = intval(WPO_WCPDF_IPS_PRO_TCPDF_STATIC::revstrpos($pmid, chr(0).chr(32).')]')) + 2;
									$spacelen = 2;
								} else {
									$pos2 = intval(WPO_WCPDF_IPS_PRO_TCPDF_STATIC::revstrpos($pmid, chr(32).')]')) + 1;
									$spacelen = 1;
								}
								if ($pos1 == $pos2) {
							...
							if (preg_match_all('/\[\(([^\)]*)\)\]/x', $pmidtemp, $lnstring, PREG_PATTERN_ORDER)) {
								...
								while (preg_match('/([0-9\.\+\-]*)[\s](Td|cm|m|l|c|re)[\s]/x', $pmid, $strpiece, PREG_OFFSET_CAPTURE, $offset) == 1) {
									...
									switch ($strpiece[2][0]) {
										...
										case 're': {
											// justify block
											if (!WPO_WCPDF_IPS_PRO_TCPDF_STATIC::empty_string($this->lispacer)) {
												$this->lispacer = '';
												break;
											}
			...
			if ($dom[$key]['tag']) {
				...
				// print list-item
				if (!WPO_WCPDF_IPS_PRO_TCPDF_STATIC::empty_string($this->lispacer) AND ($this->lispacer != '^')) {
				...
				// text
				$this->htmlvspace = 0;
				$isRTLString = preg_match(WPO_WCPDF_IPS_PRO_TCPDF_FONT_DATA::$uni_RE_PATTERN_RTL, $dom[$key]['value']) || preg_match(WPO_WCPDF_IPS_PRO_TCPDF_FONT_DATA::$uni_RE_PATTERN_ARABIC, $dom[$key]['value']);
				...
				if (!isset($dom[$key]['trimmed_space']) OR !$dom[$key]['trimmed_space']) {
					...
					if (!empty($this->HREF) AND (isset($this->HREF['url']))) {
						...
						if (($strlinelen < $cwa) AND (isset($dom[($key + 1)])) AND ($dom[($key + 1)]['tag']) AND (!$dom[($key + 1)]['block'])) {
							...
							while ($write_block AND isset($dom[$nkey])) {
								if ($dom[$nkey]['tag']) {
									...
									$nextstr = WPO_WCPDF_IPS_PRO_TCPDF_STATIC::pregSplit('/'.$this->re_space['p'].'+/', $this->re_space['m'], $dom[$nkey]['value']);
						...
						if (($wadj > 0) AND (($strlinelen + $wadj) >= $cwa)) {
							$wadj = 0;
							$nextstr = WPO_WCPDF_IPS_PRO_TCPDF_STATIC::pregSplit('/'.$this->re_space['p'].'/', $this->re_space['m'], $dom[$key]['value']);
		...
		switch($tag['value']) {
			case 'table': {
				...
				if (!isset($dom[$key]['attribute']['nested']) OR ($dom[$key]['attribute']['nested'] != 'true')) {
					$this->htmlvspace = 0;
					// set table header
					if (!WPO_WCPDF_IPS_PRO_TCPDF_STATIC::empty_string($dom[$key]['thead'])) {
						// set table header
			...
			case 'img': {
				...
				if ($imgsrc[0] === '@') {
                    ...
                    $type = WPO_WCPDF_IPS_PRO_TCPDF_IMAGES::getImageFileType($imgsrc);
                } else {
				...
				$xpos = $this->x;
				$imglink = '';
				if (isset($this->HREF['url']) AND !WPO_WCPDF_IPS_PRO_TCPDF_STATIC::empty_string($this->HREF['url'])) {
			...
			case 'li': {
				...
				if ($this->listordered[$this->listnum]) {
					// ordered item
					if (isset($parent['attribute']['type']) AND !WPO_WCPDF_IPS_PRO_TCPDF_STATIC::empty_string($parent['attribute']['type'])) {
						$this->lispacer = $parent['attribute']['type'];
					} elseif (isset($parent['listtype']) AND !WPO_WCPDF_IPS_PRO_TCPDF_STATIC::empty_string($parent['listtype'])) {
						$this->lispacer = $parent['listtype'];
					} elseif (isset($this->lisymbol) AND !WPO_WCPDF_IPS_PRO_TCPDF_STATIC::empty_string($this->lisymbol)) {
			...
			case 'input': {
				if (isset($tag['attribute']['name']) AND !WPO_WCPDF_IPS_PRO_TCPDF_STATIC::empty_string($tag['attribute']['name'])) {
				...
				$prop = array();
				$opt = array();
				if (isset($tag['attribute']['readonly']) AND !WPO_WCPDF_IPS_PRO_TCPDF_STATIC::empty_string($tag['attribute']['readonly'])) {
					$prop['readonly'] = true;
				}
				if (isset($tag['attribute']['value']) AND !WPO_WCPDF_IPS_PRO_TCPDF_STATIC::empty_string($tag['attribute']['value'])) {
					$value = $tag['attribute']['value'];
				}
				if (isset($tag['attribute']['maxlength']) AND !WPO_WCPDF_IPS_PRO_TCPDF_STATIC::empty_string($tag['attribute']['maxlength'])) {
					$opt['maxlen'] = intval($tag['attribute']['maxlength']);
				}
				$h = $this->getCellHeight($this->FontSize);
				if (isset($tag['attribute']['size']) AND !WPO_WCPDF_IPS_PRO_TCPDF_STATIC::empty_string($tag['attribute']['size'])) {
				...
				switch ($tag['attribute']['type']) {
					...
					case 'image': {
						...
						if (isset($tag['attribute']['src']) AND !WPO_WCPDF_IPS_PRO_TCPDF_STATIC::empty_string($tag['attribute']['src'])) {
			...
			case 'textarea': {
				...
				if (isset($tag['attribute']['cols']) AND !WPO_WCPDF_IPS_PRO_TCPDF_STATIC::empty_string($tag['attribute']['cols'])) {
					$w = intval($tag['attribute']['cols']) * $this->GetStringWidth(chr(32)) * 2;
				} else {
					$w = 40;
				}
				if (isset($tag['attribute']['rows']) AND !WPO_WCPDF_IPS_PRO_TCPDF_STATIC::empty_string($tag['attribute']['rows'])) {
			...
			case 'select': {
				...
				$w = 0;
				if (isset($tag['attribute']['opt']) AND !WPO_WCPDF_IPS_PRO_TCPDF_STATIC::empty_string($tag['attribute']['opt'])) {
			...
			case 'tcpdf': {
				if (defined('K_TCPDF_CALLS_IN_HTML') AND (K_TCPDF_CALLS_IN_HTML === true)) {
					// Special tag used to call WPO_WCPDF_IPS_PRO_TCPDF methods
					// This tag is disabled by default by the K_TCPDF_CALLS_IN_HTML constant on WPO_WCPDF_IPS_PRO_TCPDF configuration file.
	...
	protected function drawHTMLTagBorder($tag, $xmax) {
		...
		if (isset($tag['border']) AND !empty($tag['border'])) {
			...
			if (!WPO_WCPDF_IPS_PRO_TCPDF_STATIC::empty_string($this->thead) AND (!$this->inthead)) {
				...
				$border = WPO_WCPDF_IPS_PRO_TCPDF_STATIC::getBorderMode($border, $position='middle', $this->opencell);
			}
		}
	...
	protected function putHtmlListBullet($listdepth, $listtype='', $size=10) {
		...
		switch ($listtype) {
			...
			case 'i':
			case 'lower-roman': {
				$textitem = strtolower(WPO_WCPDF_IPS_PRO_TCPDF_STATIC::intToRoman($this->listcount[$this->listnum]));
				break;
			}
			case 'I':
			case 'upper-roman': {
				$textitem = WPO_WCPDF_IPS_PRO_TCPDF_STATIC::intToRoman($this->listcount[$this->listnum]);
				break;
			}
			case 'a':
			case 'lower-alpha':
			...
			case 'lower-greek': {
				$textitem = WPO_WCPDF_IPS_PRO_TCPDF_FONTS::unichr((945 + $this->listcount[$this->listnum] - 1), $this->isunicode);
				break;
			}
			/*
		...
		if (!WPO_WCPDF_IPS_PRO_TCPDF_STATIC::empty_string($textitem)) {
	...
	/**
	 * Returns current graphic WPO_WCPDF_IPS_PRO_variables as array.
	 * @return array of graphic WPO_WCPDF_IPS_PRO_variables
	 * @protected
	 * @since 4.2.010 (2008-11-14)
	 */
	...
	/**
	 * Set graphic WPO_WCPDF_IPS_PRO_variables.
	 * @param array $gvars array of graphic variablesto restore
	 * @param boolean $extended if true restore extended graphic WPO_WCPDF_IPS_PRO_variables
	 * @protected
	 * @since 4.2.010 (2008-11-14)
	 */
	...
	public function addTOC($page=null, $numbersfont='', $filler='.', $toc_name='TOC', $style='', $color=array(0,0,0)) {
		...
		$page_fill_end = false;
		$current_column = $this->current_column;
		if (WPO_WCPDF_IPS_PRO_TCPDF_STATIC::empty_string($numbersfont)) {
			$numbersfont = $this->default_monospaced_font;
		}
		if (WPO_WCPDF_IPS_PRO_TCPDF_STATIC::empty_string($filler)) {
			$filler = ' ';
		}
		...
		if (!WPO_WCPDF_IPS_PRO_TCPDF_STATIC::empty_string($page)) {
			for ($p = $page_first; $p <= $page_last; ++$p) {
				...
				for ($n = 1; $n <= $maxpage; ++$n) {
					...
					$na = WPO_WCPDF_IPS_PRO_TCPDF_STATIC::formatTOCPageNumber(($this->starting_page_number + $np - 1));
					$nu = WPO_WCPDF_IPS_PRO_TCPDF_FONTS::UTF8ToUTF16BE($na, false, $this->isunicode, $this->CurrentFont);
					// replace aliases with numbers
					foreach ($pnalias['u'] as $u) {
						$sfill = str_repeat($filler, max(0, (strlen($u) - strlen($nu.' '))));
						if ($this->rtl) {
							$nr = $nu.WPO_WCPDF_IPS_PRO_TCPDF_FONTS::UTF8ToUTF16BE(' '.$sfill, false, $this->isunicode, $this->CurrentFont);
						} else {
							$nr = WPO_WCPDF_IPS_PRO_TCPDF_FONTS::UTF8ToUTF16BE($sfill.' ', false, $this->isunicode, $this->CurrentFont).$nu;
						}
	...
	/**
	 * Stores a copy of the current WPO_WCPDF_IPS_PRO_TCPDF object used for undo operation.
	 * @public
	 * @since 4.5.029 (2009-03-19)
	 */
	public function startTransaction() {
		...
		$this->objcopy = WPO_WCPDF_IPS_PRO_TCPDF_STATIC::objclone($this);
	}

	/**
	 * Delete the copy of the current WPO_WCPDF_IPS_PRO_TCPDF object used for undo operation.
	 * @public
	 * @since 4.5.029 (2009-03-19)
	 */
	...
	/**
	 * This method allows to undo the latest transaction by returning the latest saved WPO_WCPDF_IPS_PRO_TCPDF object with startTransaction().
	 ...
	 * @return WPO_WCPDF_IPS_PRO_TCPDF object.
	 * @public
	 * @since 4.5.029 (2009-03-19)
	 */
	...
	public function selectColumn($col=null) {
		if (WPO_WCPDF_IPS_PRO_TCPDF_STATIC::empty_string($col)) {
	...
	*/
	...
	protected function hyphenateWord($word, $patterns, $dictionary=array(), $leftmin=1, $rightmin=2, $charmin=1, $charmax=8) {
		...
		$word_string = WPO_WCPDF_IPS_PRO_TCPDF_FONTS::UTF8ArrSubString($word, '', '', $this->isunicode);
		...
		if (isset($dictionary[$word_string])) {
			return WPO_WCPDF_IPS_PRO_TCPDF_FONTS::UTF8StringToArray($dictionary[$word_string], $this->isunicode, $this->CurrentFont);
		}
		...
		for ($pos = 0; $pos < $maxpos; ++$pos) {
			...
			for ($i = 1; $i <= $imax; ++$i) {
				$subword = strtolower(WPO_WCPDF_IPS_PRO_TCPDF_FONTS::UTF8ArrSubString($tmpword, $pos, ($pos + $i), $this->isunicode));
				if (isset($patterns[$subword])) {
					$pattern = WPO_WCPDF_IPS_PRO_TCPDF_FONTS::UTF8StringToArray($patterns[$subword], $this->isunicode, $this->CurrentFont);
	...
	public function hyphenateText($text, $patterns, $dictionary=array(), $leftmin=1, $rightmin=2, $charmin=1, $charmax=8) {
		...
		if (!is_array($patterns)) {
			$patterns = WPO_WCPDF_IPS_PRO_TCPDF_STATIC::getHyphenPatternsFromTEX($patterns);
		}
		// get array of characters
		$unichars = WPO_WCPDF_IPS_PRO_TCPDF_FONTS::UTF8StringToArray($text, $this->isunicode, $this->CurrentFont);
		// for each char
		foreach ($unichars as $char) {
			if ((!$intag) AND (!$skip) AND WPO_WCPDF_IPS_PRO_TCPDF_FONT_DATA::$uni_type[$char] == 'L') {
				// letter character
				$word[] = $char;
			} else {
				// other type of character
				if (!WPO_WCPDF_IPS_PRO_TCPDF_STATIC::empty_string($word)) {
					// hypenate the word
		...
		return WPO_WCPDF_IPS_PRO_TCPDF_FONTS::UTF8ArrSubString($txtarr, '', '', $this->isunicode);
	}
	...
	public function printTemplate($id, $x=null, $y=null, $w=0, $h=0, $align='', $palign='', $fitonpage=false) {
		...
		if (!empty($this->xobjects[$id]['annotations'])) {
			foreach ($this->xobjects[$id]['annotations'] as $annot) {
				...
				$coordlt = WPO_WCPDF_IPS_PRO_TCPDF_STATIC::getTransformationMatrixProduct($tm, array(1, 0, 0, 1, ($annot['x'] * $this->k), (-$annot['y'] * $this->k)));
				$ax = ($coordlt[4] / $this->k);
				$ay = ($this->h - $h - ($coordlt[5] / $this->k));
				$coordrb = WPO_WCPDF_IPS_PRO_TCPDF_STATIC::getTransformationMatrixProduct($tm, array(1, 0, 0, 1, (($annot['x'] + $annot['w']) * $this->k), ((-$annot['y'] - $annot['h']) * $this->k)));
	...
	protected function setSVGStyles($svgstyle, $prevsvgstyle, $x=0, $y=0, $w=1, $h=1, $clip_function='', $clip_params=array()) {
		...
		// color
		$fill_color = WPO_WCPDF_IPS_PRO_TCPDF_COLORS::convertHTMLColorToDec($svgstyle['color'], $this->spot_colors);
		$this->setFillColorArray($fill_color);
		// text color
		$text_color = WPO_WCPDF_IPS_PRO_TCPDF_COLORS::convertHTMLColorToDec($svgstyle['text-color'], $this->spot_colors);
		...
		if (preg_match('/url\([\s]*\#([^\)]*)\)/si', $svgstyle['fill'], $regs)) {
			...
			$fill_color = WPO_WCPDF_IPS_PRO_TCPDF_COLORS::convertHTMLColorToDec($svgstyle['fill'], $this->spot_colors);
		...
		if ($svgstyle['stroke'] != 'none') {
			...
			$stroke_style = array(
				'color' => WPO_WCPDF_IPS_PRO_TCPDF_COLORS::convertHTMLColorToDec($svgstyle['stroke'], $this->spot_colors),
					...
					foreach ($params as $ck => $cp) {
						...
						if ((($ck + 1) % 7) == 0) {
							...
							if ((abs($x0 - $x) < $minlen) AND (abs($y0 - $y) < $minlen)) {
								...
								// get angles
								$angs = WPO_WCPDF_IPS_PRO_TCPDF_STATIC::getVectorsAngle(1, 0, (($xa - $cax) / $rx), (($cay - $ya) / $ry));
								$dang = WPO_WCPDF_IPS_PRO_TCPDF_STATIC::getVectorsAngle((($xa - $cax) / $rx), (($ya - $cay) / $ry), ((-$xa - $cax) / $rx), ((-$ya - $cay) / $ry));
								if (($fs == 0) AND ($dang > 0)) {
									$dang -= (2 * M_PI);
	...
	protected function startSVGElementHandler($parser, $name, $attribs, $ctm=array()) {
		...
		if (isset($attribs['style']) AND !WPO_WCPDF_IPS_PRO_TCPDF_STATIC::empty_string($attribs['style']) AND ($attribs['style'][0] != ';')) {
			// fix style for regular expression
			$attribs['style'] = ';'.$attribs['style'];
		}
		foreach ($prev_svgstyle as $key => $val) {
			if (in_array($key, WPO_WCPDF_IPS_PRO_TCPDF_IMAGES::$svginheritprop)) {
				// inherit previous value
				$svgstyle[$key] = $val;
			}
			if (isset($attribs[$key]) AND !WPO_WCPDF_IPS_PRO_TCPDF_STATIC::empty_string($attribs[$key])) {
			...
			} elseif (isset($attribs['style']) AND !WPO_WCPDF_IPS_PRO_TCPDF_STATIC::empty_string($attribs['style'])) {
				// CSS style syntax
				$attrval = array();
		...
		if (isset($attribs['transform']) AND !empty($attribs['transform'])) {
			$tm = WPO_WCPDF_IPS_PRO_TCPDF_STATIC::getTransformationMatrixProduct($tm, WPO_WCPDF_IPS_PRO_TCPDF_STATIC::getSVGTransformMatrix($attribs['transform']));
		}
		...
		switch($name) {
			...
			case 'svg': {
				...
				$tm = WPO_WCPDF_IPS_PRO_TCPDF_STATIC::getTransformationMatrixProduct($tm, array( 1, 0, 0, 1, $svgX, $svgY));
				...
				if (isset($attribs['viewBox'])) {
					...
					if (sizeof($tmp) == 4) {
						...
						$tm = WPO_WCPDF_IPS_PRO_TCPDF_STATIC::getTransformationMatrixProduct($tm, $newtm);
						$this->SVGTransform($tm);
					}
				}
			...
			case 'g': {
				...
				$tm = WPO_WCPDF_IPS_PRO_TCPDF_STATIC::getTransformationMatrixProduct($tm, array($w, 0, 0, $h, $x, $y));
				$this->SVGTransform($tm);
				$this->setSVGStyles($svgstyle, $prev_svgstyle);
				break;
			}
			case 'linearGradient': {
				...
				if (isset($attribs['gradientTransform'])) {
					$this->svggradients[$this->svggradientid]['gradientTransform'] = WPO_WCPDF_IPS_PRO_TCPDF_STATIC::getSVGTransformMatrix($attribs['gradientTransform']);
				}
			...
			case 'stop': {
				...
				$stop_color = isset($svgstyle['stop-color'])?WPO_WCPDF_IPS_PRO_TCPDF_COLORS::convertHTMLColorToDec($svgstyle['stop-color'], $this->spot_colors):'black';
			...
			case 'image': {
				...
				if (!$clipping) {
					...
					if (preg_match('/^data:image\/[^;]+;base64,/', $img, $m) > 0) {
						...
						// fix image path
						if (!WPO_WCPDF_IPS_PRO_TCPDF_STATIC::empty_string($this->svgdir) AND (($img[0] == '.') OR (basename($img) == $img))) {
					...
					// get image type
					$imgtype = WPO_WCPDF_IPS_PRO_TCPDF_IMAGES::getImageFileType($img);
					if (($imgtype == 'eps') OR ($imgtype == 'ai')) {
    ...
    protected function getCachedFileContents($file)
    {
        if (!isset($this->fileContentCache[$file])) {
            $this->fileContentCache[$file] = WPO_WCPDF_IPS_PRO_TCPDF_STATIC::fileGetContents($file);
        }
    ...
    protected function fileExists($file)
    {
        ...
        return WPO_WCPDF_IPS_PRO_TCPDF_STATIC::file_exists($file);
    }

} // END OF WPO_WCPDF_IPS_PRO_TCPDF CLASS
