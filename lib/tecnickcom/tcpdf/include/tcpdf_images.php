...
//
// Description :
//   Static image methods used by the WPO_WCPDF_IPS_PRO_TCPDF class.
//
//============================================================+

/**
 * @file
 * This is a PHP class that contains static image methods for the WPO_WCPDF_IPS_PRO_TCPDF class.<br>
...
/**
 * @class WPO_WCPDF_IPS_PRO_TCPDF_IMAGES
 * Static image methods used by the WPO_WCPDF_IPS_PRO_TCPDF class.
 * @package com.tecnick.tcpdf
...
// - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
	...
	public static function _parsejpeg($file) {
		...
		if (!@WPO_WCPDF_IPS_PRO_TCPDF_STATIC::file_exists($file)) {
			return false;
		}
		$a = getimagesize($file);
		if (empty($a)) {
		...
		while (($pos = strpos($data, "ICC_PROFILE\0", $offset)) !== false) {
			...
			$length = (WPO_WCPDF_IPS_PRO_TCPDF_STATIC::_getUSHORT($data, ($pos - 2)) - 16);
	...
	public static function _parsepng($file) {
		...
		$w = WPO_WCPDF_IPS_PRO_TCPDF_STATIC::_freadint($f);
		$h = WPO_WCPDF_IPS_PRO_TCPDF_STATIC::_freadint($f);
		$bpc = ord(fread($f, 1));
		$ct = ord(fread($f, 1));
		if ($ct == 0) {
			$colspace = 'DeviceGray';
		...
		$data = '';
		$icc = false;
		$n = WPO_WCPDF_IPS_PRO_TCPDF_STATIC::_freadint($f);
		do {
			$type = fread($f, 4);
			if ($type == 'PLTE') {
				// read palette
				$pal = WPO_WCPDF_IPS_PRO_TCPDF_STATIC::rfread($f, $n);
				fread($f, 4);
			} elseif ($type == 'tRNS') {
				// read transparency info
				$t = WPO_WCPDF_IPS_PRO_TCPDF_STATIC::rfread($f, $n);
				if ($ct == 0) { // DeviceGray
					$trns = array(ord($t[1]));
				...
				$data .= WPO_WCPDF_IPS_PRO_TCPDF_STATIC::rfread($f, $n);
				fread($f, 4);
			} elseif ($type == 'iCCP') {
				// skip profile name
				$len = 0;
				...
				$icc = WPO_WCPDF_IPS_PRO_TCPDF_STATIC::rfread($f, ($n - $len - 2));
				// decompress profile
				...
				WPO_WCPDF_IPS_PRO_TCPDF_STATIC::rfread($f, $n + 4);
			}
			$n = WPO_WCPDF_IPS_PRO_TCPDF_STATIC::_freadint($f);
		} while ($n);
...
} // END OF WPO_WCPDF_IPS_PRO_TCPDF_IMAGES CLASS
