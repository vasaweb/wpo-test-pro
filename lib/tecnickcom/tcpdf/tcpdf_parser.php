...
/**
 * @class WPO_WCPDF_IPS_PRO_TCPDF_PARSER
 * This is a PHP class for parsing PDF documents.<br>
 * @package com.tecnick.tcpdf
...
// -----------------------------------------------------------------------------
	...
	protected function decodeStream($sdic, $stream) {
		...
		foreach ($filters as $filter) {
			if (in_array($filter, WPO_WCPDF_IPS_PRO_TCPDF_FILTERS::getAvailableFilters())) {
				try {
					$stream = WPO_WCPDF_IPS_PRO_TCPDF_FILTERS::decodeFilter($filter, $stream);
				} catch (Exception $e) {
					$emsg = $e->getMessage();
	...
	public function Error($msg) {
		if ($this->cfg['die_for_errors']) {
			die('<strong>WPO_WCPDF_IPS_PRO_TCPDF_PARSER ERROR: </strong>'.$msg);
		} else {
			throw new Exception('WPO_WCPDF_IPS_PRO_TCPDF_PARSER ERROR: '.$msg);
		}
	}
